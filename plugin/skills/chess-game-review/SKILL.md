...
If they're absent, tell the user to start the server (`docker start chess-mcp-server` or
`docker compose up -d` in the chess-mcp repo) — do **not** fall back to analyzing the game from
memory. The whole point is to not guess.
