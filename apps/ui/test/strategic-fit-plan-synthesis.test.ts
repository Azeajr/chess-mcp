...
test("canonical validation rejects a plan whose sections cite nothing", () => {
  ...
  assert.equal(
    check({
      ...identity,
      plan: { title: "Plan", sections: [{ kind: "strategic-plan", text: "Develop." }] },
    }).reason,
  ...
  assert.equal(
    check({
      ...
      plan: {
        title: "Plan",
        sections: [
          { kind: "model-position", text: "Play it.", concept_ids: ["concept:center-control"] },
        ],
      },
    }).reason,
  ...
  assert.equal(
    check({
      ...identity,
      plan: { title: "Plan", sections: [{ kind: "brilliancy", text: "x", concept_ids: ["c"] }] },
    }).ok,
    false,
...
test("the evidence basis discloses only deterministic material and says what it withheld", () => {
  ...
  assert.equal(
    basis.moves.includes("Qh5"),
    false,
    "the vocabulary is the validated paths, not chess",
  );
...
test("a grounded plan stages without touching training metadata, resolutions, or the repertoire", () => {
  ...
  assert.deepEqual(
    proposal.sections.map((section) => section.kind),
    ["strategic-plan", "danger-sign"],
  );
  assert.deepEqual(
    proposal.sections[0]!.cited_moves,
    ["Nf3"],
    "the mentioned move resolved to evidence",
  );
  assert.equal(
    JSON.stringify(proposal).includes("evidence_identity"),
    false,
...
test("accepting saves through the training writer; rejecting leaves no trace", () => {
  ...
  assert.deepEqual(subject.artifacts, []);
  assert.equal(
    subject.state.accept(rejected.plan_id).ok,
    false,
    "a rejected plan cannot be replayed",
  );
  ...
  assert.equal(
    result.ok === true && result.training_id?.startsWith("strategic-fit-training:"),
    true,
  );
  assert.equal(subject.state.plan(accepted.plan_id)?.status, "accepted");
  assert.equal(
    subject.state.accept(accepted.plan_id).ok,
    false,
  ...
  assert.match(
    resolution.note ?? "",
    /Hold the Nf3 setup/,
    "the confirmed plan reaches document metadata",
  );
  assert.match(resolution.note ?? "", /Danger sign:/);
  assert.match(
    resolution.note ?? "",
    /checkpoint:second/,
    "durable text keeps the evidence behind each section",
  );
  assert.ok(
    subject.performance().targets.length >= 2,
    "acceptance registers the existing drill targets",
  );
...
test("the saved drill artifact carries the plan card with its evidence links", () => {
  ...
  assert.equal(
    artifact.plan_card?.training_id,
    artifact.training_id,
    "the card names the item it belongs to",
  );
  assert.equal(
...
test("a plan fails closed once the revision, the evidence, or the finding moves", () => {
  for (const [label, disturb] of [
    ...
    assert.deepEqual(
      subject.metadata().training_references,
      [],
...
test("a fake model's plan reaches the browser command, stages only, and never edits the repertoire", async (t) => {
  ...
  assert.equal(
    unsupported.error,
    "invalid_arguments",
    "an unsupported model position never reaches the store",
  );
