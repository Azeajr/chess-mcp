...
test("canonical retrieval schema is shared by both hosts with host-appropriate identity", () => {
  ...
  assert.match(
    toolContract("get_strategic_fit_report").result.semantics ?? "",
...
test("retrieval arguments reject blank, cross-view, and oversized requests", () => {
  ...
  assert.equal(
    ok({
      report_id: "strategic-fit-report:a",
      view: "findings",
      page: { limit: 25 },
      sort: "finding-id",
    }).ok,
  ...
  assert.equal(
    ok({ report_id: "strategic-fit-report:a", view: "finding", finding_id: "finding:a" }).ok,
    true,
  );
  assert.equal(ok({ report_id: "   " }).reason, "report_id must not be blank");
  assert.equal(
    ok({ report_id: "r", view: "finding" }).reason,
    "view finding requires a non-blank finding_id",
  );
  assert.equal(
    ok({ report_id: "r", finding_id: "finding:a" }).reason,
    "finding_id is only valid with view finding",
  );
  assert.equal(
    ok({ report_id: "r", view: "summary", page: { limit: 5 } }).reason,
    "page is only valid with view findings",
  );
  assert.equal(
  ...
  assert.equal(
    validateToolArguments("get_strategic_fit_report", { report_id: "r" }, "mcp").ok,
    false,
  );
});
...
test("browser retrieval returns bounded summary, findings, and finding views for a cached report", async () => {
  ...
  assert.ok(
    findings.findings.every(
      (row) => row.source_san_paths.length <= STRATEGIC_FIT_CONVERSATION_LIMITS.san_paths,
    ),
  );
  ...
  assert.ok(
    detail.finding.evidence.dimensions.length <=
      STRATEGIC_FIT_CONVERSATION_LIMITS.evidence_dimensions,
  );
...
test("history compaction preserves retrieval identities and paging cursors", async () => {
  ...
  for (const identity of [
    "strategic-fit-report:abc",
    "finding:one",
    "semantic-finding:one",
    "browser:12",
    "next_cursor",
    "source_san_paths",
    "strategic-fit-findings",
  ])
