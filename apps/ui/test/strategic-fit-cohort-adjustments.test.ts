...
function fixture(initialMetadata?: StrategicFitDocumentMetadata) {
  const auto = report(
    "report:auto",
    [cohort("cohort:a", ["route:a1", "route:a2"]), cohort("cohort:b", ["route:b1"])],
    [finding("finding:a", "cohort:a", "route:a2"), finding("finding:b", "cohort:b", "route:b1")],
  );
  ...
  const graph = {
    ...
    decisions: ["decision:route:a1", "decision:route:a2", "decision:route:b1"].map(
      (decisionId) => ({
  ...
  const boundary: StrategicFitCohortAdjustmentBoundary = {
    ...
    execute: async (_command, args) => {
      executeCount++;
      const overrides = args.cohort_overrides as readonly {
        kind: string;
  ...
  return {
    ...
    setReport: (next: StrategicFitAnalysisResult) => {
      currentReport = next;
    },
  };
}
...
test("empty, duplicate, conflicting, and stale semantic changes fail closed without partial persistence", async () => {
  ...
  assert.equal(
    await conflict.state.preview(conflict.auto.report_id, {
      kind: "merge",
      route_ids: ["route:a1", "route:b1"],
    }),
    null,
