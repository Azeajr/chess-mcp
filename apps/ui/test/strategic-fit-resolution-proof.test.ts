...
import { proofClaimText, proofMakesNoResolutionClaim } from "../src/components/strategic-fit/ResolutionProof.tsx";
import type {
...
function acceptedStage(overrides: Partial<{ accepted_revision: number | null; status: string }> = {}): StrategicFitStagedChange {
  return {
...
function metric(id: string, value: number | null, state = "available", reason: string | null = null): StrategicFitMetric<number> {
  return {
    metric_id: id,
    state,
    value,
...
function reanalysisSummary(patch: Partial<StrategicFitReanalysisSummary> = {}): StrategicFitReanalysisSummary {
  return {
  ...
  return {
    ...
    result: {
      ...
      summary: {
        ...
        metrics: {
          familiarity_adjusted_coverage: metric("familiarity-adjusted-coverage", options.coverage ?? 0.96),
...
function proofHarness() {
  let lifecycle = { status: "stale", current_result: null, error: null } as unknown as StrategicFitLifecycleSnapshot;
  ...
  let undoResult: StrategicFitChangeOperationResult = {
    ok: true,
    stage: { ...acceptedStage(), status: "undone", accepted_revision: 7 } as StrategicFitStagedChange,
  };
  ...
  return {
    ...
    recordResolvers,
    undoResolvers,
    deferRecords: () => { deferRecords = true; },
    deferUndo: () => { deferUndo = true; },
    setLifecycle: (value: Partial<StrategicFitLifecycleSnapshot>) => {
      lifecycle = { status: "stale", current_result: null, error: null, ...value } as StrategicFitLifecycleSnapshot;
    },
    setRevision: (revision: number) => { current = { ...current, repertoire_revision: revision }; },
    setRecord: (value: StrategicFitUndoRecordSummary | null) => { record = value; },
    setUndoResult: (value: StrategicFitChangeOperationResult) => { undoResult = value; },
  };
}
...
test("acceptance tracks without any success claim until the affected-cohort rescan completes and proves resolution", async () => {
  ...
  const before = completedResult({ reportId: "report:before", revision: 5, findings: [finding(SEMANTIC_ID)], unresolved: 4 });
  ...
  const workload = snapshot.claims?.metrics.find((claim) => claim.claim_id === "training-adjusted-workload");
  assert.equal(workload?.before?.value, 0.4);
  assert.equal(workload?.after?.value, 0.34);
  const unresolvedCounts = snapshot.claims?.counts.find((claim) => claim.claim_id === "unresolved-finding-count");
...
test("still-unresolved rescan keeps the finding open with changed evidence and surfaces new findings from the change", async () => {
  const h = proofHarness();
  h.state.track(acceptedStage(), completedResult({ reportId: "report:before", revision: 5, findings: [finding(SEMANTIC_ID)] }));
  await settle();
  ...
  assert.equal(snapshot.outcome?.kind === "still-open" && snapshot.outcome.finding.explanation,
    "Still mismatched after the accepted change.");
  assert.deepEqual(snapshot.new_findings.map((entry) => entry.semantic_finding_id), ["semantic:new"]);
  assert.equal(snapshot.claims?.counts.find((claim) => claim.claim_id === "unresolved-finding-count")?.after, 2);
});

test("coverage-preserving change reports identical before and after coverage from complete reports only", async () => {
  const h = proofHarness();
  h.state.track(acceptedStage(), completedResult({
    reportId: "report:before", revision: 5, findings: [finding(SEMANTIC_ID)], coverage: 0.96,
  }));
  await settle();
  const after = completedResult({
    reportId: "report:after",
    ...
    findings: [],
    coverage: 0.96,
    reanalysis: reanalysisSummary({ auto_resolved_semantic_finding_ids: [SEMANTIC_ID], disappeared_semantic_finding_ids: [SEMANTIC_ID] }),
  });
  h.setLifecycle({ status: "completed", current_result: after });
  h.state.synchronize();
  const coverage = h.state.snapshot().claims?.metrics.find((claim) => claim.claim_id === "familiarity-adjusted-coverage");
...
test("a rescan completed without a reconciliation summary stays honest about the missing resolution record", async () => {
  ...
  h.setLifecycle({
    status: "completed",
    current_result: completedResult({ reportId: "report:after", revision: 6, findings: [], reanalysis: null }),
  });
  h.state.synchronize();
...
test("a race with another edit supersedes the proof and rescan failure or cancellation stays claimless", async () => {
  ...
  h.state.track(acceptedStage(), completedResult({ reportId: "report:before", revision: 5, findings: [finding(SEMANTIC_ID)] }));
  await settle();
  h.setLifecycle({ status: "failed", error: { code: "strategic_fit_engine_unavailable", message: "Engine died." } });
  h.state.synchronize();
...
test("undo from the proof runs one exact undo record and re-verifies the restored report after rescan", async () => {
  ...
  assert.equal(snapshot.claims?.counts.find((claim) => claim.claim_id === "unresolved-finding-count")?.after, 4);
...
test("blocked undo surfaces the structured error without mutating and unavailable records disable undo", async () => {
  ...
  assert.deepEqual(h.undoCalls, ["undo:proof"], "controller-authoritative rejection must come from one submitted undo");
...
test("tracking rejects unaccepted stages, reload starts claimless, and clearing forgets the acceptance", async () => {
  const h = proofHarness();
  assert.equal(h.state.track(acceptedStage({ status: "staged", accepted_revision: null }), null), false);
...
test("late undo-record and undo completions from a superseded tracking sequence never overwrite newer state", async () => {
  ...
  h.state.clear();
  h.undoResolvers[0]!({ ok: true, stage: { ...acceptedStage(), status: "undone", accepted_revision: 7 } as StrategicFitStagedChange });
...
test("proof UI supplies keyboard, screen-reader, no-color, reduced-motion, mobile, and overflow contracts", () => {
  const component = readFileSync(new URL("../src/components/strategic-fit/ResolutionProof.tsx", import.meta.url), "utf8");
