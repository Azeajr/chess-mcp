...
function portfolioFixture(options: { readonly conflicts?: boolean } = {}) {
  ...
  let revision = 4;
  let available = true;
  let outcome = { ok: true, stage_id: "stage:portfolio", code: null as string | null, message: "staged" };
  ...
  return {
    ...
    setDocument: (next: string) => { documentId = next; },
    setRevision: (next: number) => { revision = next; },
    setAvailable: (next: boolean) => { available = next; },
    setOutcome: (next: typeof outcome) => { outcome = next; },
  };
}
...
test("bounds are staged for confirmation and bind nothing until the user confirms them", () => {
  ...
  assert.deepEqual(subject.state.confirm(proposal.constraint_set_id), { ok: true, status: "confirmed" });
  ...
  assert.deepEqual(subject.state.confirm(proposal.constraint_set_id), { ok: false, status: "confirmed" },
...
test("a contradiction is reported with the question to ask and never silently applied", () => {
  ...
  assert.equal(subject.state.constraintSet(proposal.constraint_set_id)?.status, "pending",
...
test("rejected bounds persist nothing and cannot be reused", () => {
  ...
  assert.deepEqual(subject.state.reject(proposal.constraint_set_id), { ok: true, status: "rejected" });
  assert.throws(
    () => subject.state.portfolio(proposal.constraint_set_id),
    (error: { code?: string }) => error.code === "strategic_fit_portfolio_unconfirmed_constraints",
  );
...
test("confirmed bounds go stale when the document or repertoire moves under them", () => {
  ...
  assert.deepEqual(pending.state.confirm(third.constraint_set_id), { ok: false, status: "stale" },
...
test("selecting an option stages the existing change set and applies nothing", async () => {
  ...
  const selected = await subject.state.select(proposal.constraint_set_id, option.option_id) as Extract<
  ...
  assert.deepEqual(subject.stageCalls, [{ candidate_id: option.candidate_id, action: option.action }],
    "staging is delegated to the existing review path exactly once");
  assert.equal(subject.state.selection()?.status, "staged");

  const unknown = await subject.state.select(proposal.constraint_set_id, "strategic-fit-portfolio-option:invented") as
    { error: string };
  assert.equal(unknown.error, "strategic_fit_portfolio_unknown_option");
  assert.equal(subject.stageCalls.length, 1, "an option the portfolio never returned stages nothing");

  const unconfirmed = portfolioFixture();
  const pending = unconfirmed.state.propose({ constraints: LOOSE });
  const refused = await unconfirmed.state.select(pending.constraint_set_id, option.option_id) as { error: string };
...
test("a failed stage leaves nothing selected and reports the change controller's own code", async () => {
  ...
  subject.setOutcome({ ok: false, stage_id: null, code: "stale-result", message: "Replacement context is stale." });

  const failed = await subject.state.select(proposal.constraint_set_id, option.option_id) as { error: string; reason: string };
  assert.equal(failed.error, "stale-result");
  assert.equal(failed.reason, "Replacement context is stale.");
  assert.equal(subject.state.selection()?.status, "failed",
...
test("choosing a second option supersedes the first so only one change is ever staged", async () => {
  ...
  assert.equal(subject.stageCalls.length, 2,
...
test("argument shapes that could smuggle an unconfirmed bound are rejected before the store", () => {
  const invalid = (args: Record<string, unknown>) =>
    validateToolArguments("propose_strategic_fit_portfolio", args, "browser") as { ok: boolean; reason?: string };
  ...
  assert.equal(invalid({ constraints: LOOSE, constraint_set_id: "strategic-fit-portfolio-constraints:1" }).ok, false,
    "bounds cannot be stated and used in the same call, which would skip confirmation");
  assert.equal(invalid({ constraints: LOOSE, option_id: "strategic-fit-portfolio-option:candidate" }).ok, false);
  ...
  assert.equal(invalid({ constraints: { danger: 3 } }).ok, false,
...
test("a fake model's portfolio call stages only and never edits the repertoire", async (t) => {
    ...
    new ReadableStream({
      start(controller) {
        ...
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({
          choices: [{
            delta: { tool_calls: [{ index: 0, id: "p1", function: { name: "propose_strategic_fit_portfolio", arguments: portfolioArguments } }] },
  ...
  const smuggled = await executeBrowserCommand(
    ...
    { constraints: { maximum_engine_loss_cp: 30 }, constraint_set_id: "strategic-fit-portfolio-constraints:1" },
    {},
    defaultBrowserCommandDependencies,
  ) as { error: string };
  assert.equal(smuggled.error, "invalid_arguments",
  ...
  assert.equal(unconfirmed.error, "strategic_fit_portfolio_unconfirmed_constraints",
...
test("the constraint and portfolio results the model sees carry no writable state", () => {
  const subject = portfolioFixture();
  const proposal: StrategicFitPortfolioConstraintResult = subject.state.propose({ constraints: LOOSE });
  subject.state.confirm(proposal.constraint_set_id);
  const portfolio: StrategicFitPortfolioViewResult = subject.state.portfolio(proposal.constraint_set_id);
  for (const key of ["applied", "persisted", "automatic_selection"] as const) {
    if (key in portfolio) assert.equal((portfolio as unknown as Record<string, unknown>)[key], false);
  }
  for (const option of portfolio.options) {
    assert.ok(option.evidence_identity.length > 0, "an option names the retained evidence it stands on");
    ...
    for (const measurement of option.measurements) {
      assert.ok(measurement.state === "available" || measurement.value === null,
