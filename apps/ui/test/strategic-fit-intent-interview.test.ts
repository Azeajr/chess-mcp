...
function interview(initial = createDefaultStrategicFitDocumentMetadata()) {
  const documents = new Map<string, StrategicFitDocumentMetadata>([["document:a", structuredClone(initial)]]);
  ...
  return {
    ...
    setSettingsIdentity: (next: string) => { settingsIdentity = next; },
...
test("the canonical proposal contract is browser-only, action-shaped, and stages nothing", () => {
  ...
  assert.deepEqual(Object.keys(browser.properties as Record<string, unknown>).sort(), ["mode", "preferences", "rationale"]);
...
test("canonical validation rejects malformed, empty, and self-contradictory proposals", () => {
  ...
  assert.equal(check({ preferences: {} }).reason, "preferences must contain at least one preference");
  assert.equal(check({ preferences: { feature_family_weights: {} } }).reason, "preferences.feature_family_weights must contain at least one signal family");
  assert.equal(
    check({ preferences: { preferred_concept_ids: [CONCEPT], avoided_concept_ids: [CONCEPT] } }).reason,
  ...
  assert.equal(check({ rationale: "x".repeat(STRATEGIC_FIT_INTENT_LIMITS.rationale_characters + 1), mode: "balanced" }).ok, false);
});
...
test("patch resolution rejects out-of-range values and invented concepts instead of adjusting them", () => {
  ...
  assert.equal(code({ preferences: { unknown_preference: 1 } }), "strategic_fit_intent_unknown_field");
  assert.equal(code({ preferences: { feature_family_weights: { tempo: 1 } } }), "strategic_fit_intent_unknown_field");
  assert.equal(code({ preferences: { manual_weight_importance: 4 } }), "strategic_fit_intent_invalid_value");
  assert.equal(code({ preferences: { manual_weight_importance: Number.NaN } }), "strategic_fit_intent_invalid_value");
  assert.equal(code({ preferences: { maximum_engine_loss_cp: 12.5 } }), "strategic_fit_intent_invalid_value");
  assert.equal(code({ preferences: { feature_family_weights: { "pawn-topology": 9 } } }), "strategic_fit_intent_invalid_value");
  assert.equal(code({ preferences: { preferred_concept_ids: ["concept:iqp"] } }), "strategic_fit_intent_invalid_concept_id");
  assert.equal(code({ preferences: { preferred_tactical_character: ["Very Sharp"] } }), "strategic_fit_intent_invalid_value");
  assert.equal(
    code({ preferences: { preferred_concept_ids: [CONCEPT], avoided_concept_ids: [CONCEPT] } }),
    "strategic_fit_intent_conflicting_concepts",
  );
  assert.equal(code({ mode: "custom", preferences: { manual_weight_importance: 0.5 } }), "accepted");

  const patch = resolveStrategicFitIntentPatch({
    preferences: { preferred_concept_ids: [CONCEPT, CONCEPT], avoided_concept_ids: [OTHER_CONCEPT] },
    rationale: "  low theory  ",
  });
  assert.deepEqual(patch.preferences?.preferred_concept_ids, [CONCEPT], "duplicates collapse without failing");
  ...
  assert.equal(isStrategicConceptId("setup-family"), false, "a bare namespace is not a concept identity");
});
...
test("a proposal reports the exact diff and persists nothing until it is accepted", () => {
  ...
  assert.equal(proposal.confirms_provisional_profile, true, "the default profile is still provisional");
  assert.deepEqual(
    proposal.diff.map((entry) => entry.field).sort(),
    [
      "mode",
  ...
  const tolerance = proposal.diff.find((entry) => entry.field === "preferences.additional_memorization_tolerance")!;
  assert.deepEqual([tolerance.current, tolerance.proposed], [before.preferences.additional_memorization_tolerance, 0.2]);
  assert.equal(JSON.stringify(proposal).includes("profile_identity"), false, "identities stay host-side");
...
test("accepting commits through the profile state; rejecting leaves no trace", () => {
  ...
  assert.equal(session.state.accept(accepted.proposal_id).ok, false, "an accepted proposal cannot be replayed");
  ...
  assert.equal(replayed.ok === false && replayed.error, "strategic_fit_intent_proposal_not_pending");
});
...
test("a bare preset selection is applied as that preset rather than a custom profile", () => {
  ...
  assert.deepEqual(proposal.diff.map((entry) => entry.field), ["mode"]);
...
test("a value-identical proposal confirms a provisional profile but is refused once intent is explicit", () => {
  ...
  assert.equal(confirmation.confirms_provisional_profile, true, "accepting still converts the inferred default");
...
test("a proposal fails closed once the revision, settings, profile, or document moves", () => {
  for (const [label, disturb] of [
    ...
    ["settings", (session: ReturnType<typeof interview>) => session.setSettingsIdentity("settings:two")],
    ["profile", (session: ReturnType<typeof interview>) => { session.editProfileOutOfBand(); }],
    ["document", (session: ReturnType<typeof interview>) => session.setDocument("document:b")],
  ] as const) {
    const session = interview();
    const proposal = session.state.propose({ preferences: { opponent_popularity_importance: 0.8 } });
    ...
    assert.equal(session.writes(), writesBefore + (label === "profile" ? 1 : 0), `${label}: no write from the stale accept`);
  }
});
...
test("the diff compares two profiles field by field and omits everything unchanged", () => {
  ...
  const changed: StrategicFitProfile = {
    ...
    preferences: {
      ...
      feature_family_weights: { ...base.preferences.feature_family_weights, "center-dynamics": 2.5 },
    },
  };
  assert.deepEqual(
    diffStrategicFitProfiles(base, changed),
    [
...
test("a fake model's proposal reaches the browser command, stages only, and never edits the repertoire", async (t) => {
    ...
    new ReadableStream({
      start(controller) {
        ...
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({
          choices: [{
            delta: { tool_calls: [{ index: 0, id: "p1", function: { name: "propose_strategic_fit_profile", arguments: proposalArguments } }] },
  ...
  const result = await executeBrowserCommand(
    ...
    {},
    {
      ...
      proposeStrategicFitProfile: (input) => {
        staged.push(input);
        return { kind: "strategic_fit_profile_proposal", proposal_id: "strategic-fit-profile-proposal:1", status: "pending" };
      },
  ...
  assert.equal(defaultBrowserCommandDependencies.currentPgn(), treeBefore, "the repertoire PGN is unchanged");
  assert.equal(defaultBrowserCommandDependencies.currentRevision(), revisionBefore, "the document revision is unchanged");
...
test("the real browser command stages a proposal without writing profile metadata", async () => {
  ...
  const profileBefore = JSON.stringify(defaultBrowserCommandDependencies.currentStrategicFitProfile());
  const result = await executeBrowserCommand(
    "propose_strategic_fit_profile",
  ...
  assert.equal(result.diff.some((entry) => entry.field === "preferences.minimum_opponent_coverage"), true);
  assert.equal(JSON.stringify(defaultBrowserCommandDependencies.currentStrategicFitProfile()), profileBefore);
