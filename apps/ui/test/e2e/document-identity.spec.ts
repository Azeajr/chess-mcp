...
test("initial, import, edit, navigation, save, New, and failed-load identity lifecycle", async ({
  page,
}) => {
  ...
  expect(
    await chess(page, (api) => api.applyEdit("add", ["e4", "e5"], { addMoves: ["Nf3"] })),
  ...
  await expect(
    chess(page, (api) => api.loadPgn("1. e4 e5 2. e4 *", "illegal.pgn")),
...
test("separate explicit imports never share identity, even with the same name or contents", async ({
  page,
}) => {
...
test("cancelled and invalid file-picker loads leave the active identity and content untouched", async ({
  page,
}) => {
