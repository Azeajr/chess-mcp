...
const basis = (overrides: Record<string, unknown> = {}) => ({
  ...
  checkpoints: [{ checkpoint_id: "checkpoint:second", kind: "configured-ply", ply: 2, comparability: "comparable" }],
  omitted_checkpoint_count: 0,
  drills: [{ drill_id: "strategic-fit-drill:one", expected_san: "Nf3", source: "causal-move", checkpoint_id: null }],
...
test("the plan evidence card shows only deterministic material and names what was withheld", async ({ page }) => {
  await chess(page, (api, result) => api.appendToolResultForTesting("propose_strategic_fit_plan", result), basis());
  ...
  await expect(card).toContainText("1 concept · 1 checkpoint · 1 drill position · 3 validated moves");
  ...
  await expect(bounded.locator(".strategic-fit-plan-omitted")).toContainText("Withheld evidence exists; it is not absent");
});

test("a staged plan card shows each section's supporting evidence and that nothing is saved yet", async ({ page }) => {
  await chess(page, (api, result) => api.appendToolResultForTesting("propose_strategic_fit_plan", result), planCard());

  const card = page.getByRole("region", { name: "Strategic Fit plan card" });
  await expect(card).toContainText("Plan · Hold the Nf3 setup");
  await expect(card.locator('[data-section-kind="strategic-plan"]')).toContainText("Answer with Nf3 and finish development.");
  await expect(card.locator('[data-section-kind="strategic-plan"]')).toContainText("Evidence: concept:center-control · moves Nf3");
  await expect(card.locator('[data-section-kind="danger-sign"]')).toContainText("Danger sign");
  await expect(card.locator('[data-section-kind="danger-sign"]')).toContainText("Evidence: checkpoint:second");
...
test("an unsupported plan is reported as a structured refusal rather than rendered content", async ({ page }) => {
  await chess(page, (api) => api.appendToolResultForTesting("propose_strategic_fit_plan", {
    error: "strategic_fit_plan_unsupported_move",
    reason: "plan.sections[0].text mentions f5, which is not a move on any validated path for this finding.",
  }));
