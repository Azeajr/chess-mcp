...
test("an ambiguous natural request needs no preset and direct analysis remains available", async ({
  page,
}) => {
  await chess(page, (api) => api.loadPgn("1. e4 (1. d4 d5) e5 2. Nf3", "prep.pgn"));
  await expect(
    page.getByPlaceholder("Ask about this position, game, or repertoire…"),
  ).toBeVisible();
...
test("direct repertoire previews expose cancel and accept controls", async ({ page }) => {
  ...
  await expect(page.getByRole("status", { name: "Staged repertoire line" })).toContainText(
    "1. d4 d5",
  );
...
test("stale staged edits are refused after the document changes", async ({ page }) => {
  ...
  expect(await chess(page, (api, editId) => api.acceptStagedEdit(editId), id)).toEqual({
    ok: false,
...
test("artifact saving is a browser download affordance", async ({ page }) => {
  const artifact = await chess(page, (api) =>
    api.createArtifact("pgn", "1. e4 *", "annotated.pgn"),
  );
...
test("typed audit navigation and nested only-move deck saving work from chat results", async ({
  page,
}) => {
  await chess(page, (api) => api.loadPgn("1. e4 e5 2. Nf3 Nc6"));
  ...
  const deck = await chess(page, (api) =>
    api.createArtifact("csv", "front,back\nposition,e4", "only-move-drill.csv"),
  );
  await chess(
    page,
    (api, artifact) =>
      api.appendToolResultForTesting("find_only_moves", {
        positions_scanned: 4,
        only_moves_found: 1,
        findings: [],
        lines: [],
  ...
  const downloadPromise = page.waitForEvent("download");
  await page
    .locator(".tool-result .artifact-card")
    .last()
...
test("all newly reachable primary reports have typed chat cards", async ({ page }) => {
  await chess(page, (api) =>
    api.appendToolResultForTesting("find_structures", {
      total_matches: 2,
      leaves_total: 8,
      matches: [{ path: ["d4", "d5"], structure: "Carlsbad" }],
    }),
  );
  await chess(page, (api) =>
    api.appendToolResultForTesting("prep_vs_opponent", {
      username: "alice",
      games_matched_color: 12,
      coverage_pct: 75,
      uncovered_opponent_moves: [],
    }),
  );
  const artifact = await chess(page, (api) =>
    api.createArtifact("pgn", "1. d4 *", "prep-annotated.pgn"),
  );
  await chess(
    page,
    (api, nested) =>
      api.appendToolResultForTesting("export_annotated_repertoire", {
        annotated: { audit: 1, only_moves: 2, gaps: 3, congruence: 4 },
        artifact: nested,
      }),
    artifact,
  );
...
test("top-level chat artifacts have a save affordance", async ({ page }) => {
  const artifact = await chess(page, (api) =>
    api.createArtifact("pgn", "1. e4 *", "review-annotated.pgn"),
  );
  await chess(
    page,
    (api, result) => api.appendToolResultForTesting("export_annotated_pgn", result),
    artifact,
  );
  const downloadPromise = page.waitForEvent("download");
  await page
...
test("structured command errors render as distinct result cards", async ({ page }) => {
  await chess(page, (api) =>
    api.appendToolResultForTesting("find_structures", {
      ...
      reason: "provide a structure or theme",
    }),
  );
