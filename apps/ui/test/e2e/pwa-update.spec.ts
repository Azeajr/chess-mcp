...
/** WP-019 — prompt-driven PWA update flow through the dev-only waiting-worker simulation. */
...
test("WP-019 AC-1 a waiting update offers Reload and Later without activating itself", async ({
  ...
  // Merely detecting and rendering the update must not ask the worker to activate or reload.
  ...
  // In development there is no service worker; the seam performs the same final reload and leaves
  // a sessionStorage receipt so the assertion survives the navigation.
  await Promise.all([
...
test("WP-019 AC-3 Later dismisses this page but a still-pending update returns on reload", async ({
  ...
  // Later leaves the waiting update pending; it only records the choice in page memory.
  expect(await pwa(page, (api) => api.pwaUpdateSnapshotForTesting())).toMatchObject({
    pending: true,
    visible: false,
    reloadRequested: false,
