...
test("six frozen phases expose current, completed, pending, cancelled, and reduced-motion states", async ({
  page,
}) => {
  ...
  await expect(dialog.getByRole("status")).toContainText(
    "Current phase: Identifying comparable branches",
  );
...
test("empty input blocks after normalization and never claims dependent phases ran", async ({
  page,
}) => {
  await bootstrap(page);
  ...
  await expect(dialog.locator("[data-preflight-code='empty-repertoire']")).toContainText(
    "Empty repertoire",
  );
  await expect(dialog.locator("[data-preflight-code='empty-repertoire']")).toContainText(
    "Blocking",
...
test("small, shallow, incomplete, and insufficient evidence remains a meaningful degraded report", async ({
  ...
  await expect(dialog.locator("[data-preflight-code='single-route']")).toContainText(
    "Evidence limitation",
  );
...
test("transpositions, terminal routes, and offline opening evidence remain visibly qualified", async ({
  ...
  await expect(
    transpositionEvidence.getByText("Affected repertoire paths", { exact: true }),
  ).toBeVisible();
  await expect(
    dialog.locator("[data-preflight-code='missing-opening-classification']"),
...
test("custom-start and malformed blocking evidence is explicit and withholds unsafe counts", async ({
  page,
}) => {
  ...
  await expect(dialog.locator("[data-preflight-code='unsupported-custom-start']")).toContainText(
    "Unsupported custom starting position",
  );
  await expect(dialog.locator("[data-preflight-code='malformed-data']")).toContainText(
    "Malformed repertoire data",
  );
  await expect(
...
test("ready preflight is explicit about analyzability without becoming a quality verdict", async ({
  page,
}) => {
  await bootstrap(page);
