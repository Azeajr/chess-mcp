...
const portfolio = (overrides: Record<string, unknown> = {}) => ({
  ...
  eliminations: [{
    ...
    explanation: "Evaluation loss from best, repertoire POV is 55 centipawns, outside the requested 30.",
  }],
...
test("redesign bounds are shown for confirmation and state that they bind nothing", async ({ page }) => {
  await chess(page, (api, result) => api.appendToolResultForTesting("propose_strategic_fit_portfolio", result), constraints());
  ...
  await expect(card.locator('[data-constraint-kind="maximum_engine_loss_cp"]')).toContainText("at most 30 centipawns");
  await expect(card.locator('[data-constraint-kind="minimum_expected_opponent_coverage"]')).toContainText("at least 0.8");
...
test("a contradiction is presented as the user's question and never resolved for them", async ({ page }) => {
  await chess(page, (api, result) => api.appendToolResultForTesting("propose_strategic_fit_portfolio", result), constraints({
    conflicts: [{
      source: "declared-preference",
      constraint_kinds: ["maximum_engine_loss_cp"],
      explanation: "The request accepts up to 30 centipawns of loss, but the confirmed profile allows at most 15.",
      question: "Should this redesign use the wider tolerance just this once, or should the profile change?",
    }],
  }));

  const card = page.getByRole("region", { name: "Strategic Fit redesign bounds" });
  await expect(card.locator(".strategic-fit-portfolio-conflicts")).toContainText("1 contradiction to settle");
  ...
  await expect(conflict).toContainText("Should this redesign use the wider tolerance just this once");
  await expect(card.locator('[data-constraint-kind="maximum_engine_loss_cp"]')).toContainText("at most 30 centipawns");
});
...
test("every portfolio option shows the measured value behind each bound and selects nothing", async ({ page }) => {
  await chess(page, (api, result) => api.appendToolResultForTesting("propose_strategic_fit_portfolio", result), portfolio());
  ...
  await expect(option.locator('[data-measurement-kind="maximum_engine_loss_cp"]')).toContainText("20 centipawns");
  await expect(option.locator('[data-measurement-kind="maximum_engine_loss_cp"]')).toContainText("(bound 30)");
  await expect(option.locator('[data-measurement-kind="maximum_memorization_burden"]')).toContainText("not measured");
  await expect(card).toContainText("Nothing is selected and nothing is applied.");
  await expect(card.locator(".strategic-fit-portfolio-binding")).toContainText("maximum_engine_loss_cp");
  await expect(card.locator('[data-candidate-id="candidate:costly"]')).toContainText("outside the requested 30");
});

test("an infeasible portfolio names the binding bound instead of offering a line", async ({ page }) => {
  await chess(page, (api, result) => api.appendToolResultForTesting("propose_strategic_fit_portfolio", result), portfolio({
    status: "infeasible",
    explanation: "No candidate satisfies every requested bound. Evaluation loss from best, repertoire POV alone excluded 2 candidate(s). Ask the user which bound to move.",
    options: [],
...
test("withheld options are disclosed as withheld rather than read as the whole portfolio", async ({ page }) => {
  await chess(page, (api, result) => api.appendToolResultForTesting("propose_strategic_fit_portfolio", result), portfolio({
  ...
  const card = page.getByRole("region", { name: "Strategic Fit redesign portfolio" });
  await expect(card.locator(".strategic-fit-portfolio-omitted").first()).toContainText("3 further qualifying option(s) withheld");
  await expect(card.locator(".strategic-fit-portfolio-omitted").first()).toContainText("They exist and are not");
  await expect(card.locator(".strategic-fit-portfolio-omitted").last()).toContainText("2 further exclusion(s) withheld");
});

test("an unconfirmed constraint set is reported as a structured refusal rather than a portfolio", async ({ page }) => {
  await chess(page, (api) => api.appendToolResultForTesting("propose_strategic_fit_portfolio", {
  ...
  await expect(page.getByRole("region", { name: "Strategic Fit redesign portfolio" })).toHaveCount(0);
});
