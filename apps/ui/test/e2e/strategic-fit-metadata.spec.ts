...
test("semantic resolutions and overrides persist, isolate, and stale after a referenced move edit", async ({
  page,
}) => {
  ...
  const resolutionState = (await chess(
    page,
    (api, input) => api.upsertStrategicFitResolution(input),
    {
  ...
  expect(
    (
      (await chess(page, (api) =>
        api.upsertStrategicFitCohortOverride({
  ...
  await expect
    .poll(async () => {
      const saved = (await idbValue(page, "workingRepertoire")) as
        | { documentId?: string }
        | undefined;
  ...
  expect(restored.manual_weights.route_weights[0]).toMatchObject({
    weight: 4,
    record_state: "active",
  });

  const edit = (await chess(
    page,
    (api, path) => api.applyEdit("prune", path),
    route.source_san_paths[0],
  )) as {
    ok: boolean;
  };
  ...
  expect(
    ((await chess(page, (api) => api.strategicFitMetadata())) as Metadata).resolutions,
  ).toEqual([]);
});

test("profile and resolution metadata survive reload under the same stable document ID", async ({
  page,
}) => {
...
test("custom profile preferences persist across reload without changing repertoire content or revision", async ({
  page,
}) => {
  ...
  const selected = (await chess(page, (api) =>
    api.selectStrategicFitProfile("custom", {
...
test("provisional inference is session-only and document-scoped until explicit confirmation", async ({
  page,
}) => {
  ...
  expect(
    await chess(page, (api) =>
      api.applyInferredStrategicFitProfile("versatile", {
  ...
  await chess(page, (api) =>
    api.applyInferredStrategicFitProfile("custom", {
  ...
  await chess(page, (api) =>
    api.applyInferredStrategicFitProfile("familiar-plans", {
...
test("New and import expose defaults immediately without deleting another document record", async ({
  page,
}) => {
...
test("corrupt persisted metadata falls back visibly with structured issues and is repaired", async ({
  page,
}) => {
  ...
  await expect(page.getByRole("alert")).toContainText(
    "Strategic Fit settings could not be restored",
  );
  await expect.poll(() => chess(page, (api) => api.strategicFitMetadataStatus())).toBe("ready");
