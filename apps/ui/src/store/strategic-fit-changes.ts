...
export type StrategicFitChangeOperationResult =
  ...
  | {
      readonly ok: false;
      readonly error: StrategicFitChangeErrorCode;
...
export function strategicFitChangeConfirmation(
...
function metadataWithAcceptance(
  ...
  const archiveById = new Map(
    metadata.archive_references.map((entry) => [entry.archive_id, clone(entry)]),
  );
      ...
      resolution.semantic_finding_id === stage.safety.semantic_finding_id
        ? {
            ...
            linked_staged_edit_ids: [
              ...new Set([...resolution.linked_staged_edit_ids, stage.stage_id]),
            ].sort(),
            updated_at: timestamp,
          }
        : clone(resolution),
    ),
    archive_references: [...archiveById.values()].sort((left, right) =>
...
function snapshotMatches(
  current: StrategicFitDocumentSnapshot,
  stage: StrategicFitStagedChange,
): StrategicFitChangeErrorCode | null {
  if (current.document_id !== stage.document_id) return "stale-document";
  if (
    current.revision !== stage.base_revision ||
    repertoireRevision(current.revision) !== stage.base_repertoire_revision
  )
  ...
  if (
    identity(stage.safety) !== stage.safety_identity ||
    identity(stage.change_set) !== stage.change_set_identity
  )
    return "stale-change-set";
  if (identity(stage.preview) !== stage.preview_identity) return "stale-result";
  if (identity(stage.preview.result.preview.archive_payloads) !== stage.archive_identity)
    return "archive-mismatch";
  if (identity(stageProvenance(stage)) !== stage.provenance_identity) return "provenance-mismatch";
  if (
    stage.change_set.change_set_id !== stage.preview.change_set_id ||
...
export function createStrategicFitChangeController(
  dependencies: StrategicFitChangeControllerDependencies,
) {
  ...
  const stateFor = async (documentIdValue: string) => {
    ...
    if (!load) {
      load = dependencies.storage
        .load(documentIdValue)
  ...
  const persist = async (
    ...
    await dependencies.storage.commit({
      state,
      working: working(snapshot, pgn, navigation, revision),
      metadata,
    });
  };
  ...
  return {
    ...
    async archives(): Promise<readonly StrategicFitStoredArchive[]> {
      try {
        return clone((await stateFor(dependencies.current().document_id)).archives);
      } catch {
        return [];
      }
    },
    async archivePayload(archiveId: string): Promise<ReplacementArchivePayload | null> {
      ...
      try {
        state = await stateFor(dependencies.current().document_id);
      } catch {
        return null;
      }
    ...
    async stageChangeSet(input: {
      ...
      try {
        await stateFor(current.document_id);
      } catch {
        return { ok: false, error: "archive-mismatch", stage: null };
      }
      ...
      if (applied.status !== "success") {
        const code: StrategicFitChangeErrorCode =
          applied.output.status === "stale" ? "stale-revision" : "apply-failed";
    ...
    reject(stageId: string): Promise<StrategicFitChangeOperationResult> {
      return exclusive(async () => {
        ...
        if (stage.status !== "staged")
          return { ok: false, error: "already-finalized", stage: clone(stage) };
    ...
    accept(stageId: string): Promise<StrategicFitChangeOperationResult> {
      return exclusive(async () => {
        ...
        if (stage.status === "accepted")
          return { ok: false, error: "already-accepted", stage: clone(stage) };
        if (stage.status !== "staged")
          return { ok: false, error: "already-finalized", stage: clone(stage) };
        const current = dependencies.current();
        ...
        const archiveById = new Map(
          priorState.archives.map((entry) => [entry.payload.archive_id, clone(entry)]),
        );
        for (const payload of applied.output.result.preview.archive_payloads) {
          ...
          archiveById.set(payload.archive_id, {
            payload: clone(payload),
            archived_by_stage_id: stageId,
            status: "archived",
          });
        }
        const navigation = currentPathFor(applied.tree, stage.navigation_san_path);
        const archives = [...archiveById.values()].sort((left, right) =>
          left.payload.archive_id.localeCompare(right.payload.archive_id),
        );
        const undo: StrategicFitUndoRecord = {
          ...
          before: {
            ...
            navigation: clone(current.navigation),
            archives: clone(priorState.archives),
          },
          after: {
            pgn: applied.output.result.pgn,
    ...
    undo(undoId?: string): Promise<StrategicFitChangeOperationResult> {
      return exclusive(async () => {
        ...
        try {
          priorState = clone(await stateFor(current.document_id));
        } catch {
          return { ok: false, error: "archive-mismatch", stage: null };
        }
        const record = undoId
        ...
        if (
          current.revision !== record.accepted_revision ||
          current.pgn !== record.after.pgn ||
          identity(current.metadata) !== identity(record.after.metadata) ||
          currentArchives !== identity(record.after.archives)
        ) {
          const undo = priorState.undo.map((entry) =>
        ...
        const nextUndo = priorState.undo.map((entry) => {
          ...
          const rebasesToRestoredSnapshot =
  ...
  return {
    ...
    commit: ({ state, working: saved, metadata }) =>
      idbMutateAtomically([
        ...
        {
          key: `${STRATEGIC_FIT_METADATA_STORAGE_KEY_PREFIX}${state.document_id}`,
...
const browserController = createStrategicFitChangeController({
  ...
  publish: (tree, metadata, navigation, expectedRevision) => {
    let result: { ok: true; revision: number } | { ok: false; error: string } = {
      ok: false,
    ...
    batch(() => {
      result = browserDocumentMutationRegistry.strategic_fit_change_set.publish(
        ...
        (input) =>
          actions.applyStrategicFitSnapshot(input.tree, input.navigation, input.expected_revision),
      );
  ...
  rollback: (snapshot) => {
    ...
    batch(() => {
      browserDocumentMutationRegistry.strategic_fit_change_set.rollback(
        ...
        (input) => {
          actions.restoreStrategicFitSnapshot(
            input.tree,
            input.navigation,
            input.revision,
            input.dirty,
