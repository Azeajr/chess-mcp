...
8. Interlink the repertoire: find_transposition_opportunities → move-order bridges (frontier_link = extend a stopped line 1 ply into existing prep; move_order_merge = an alternate order; coverage_confirmed = an opponent try already covered). Engine-free; the link is sound by construction (it lands in your own prep). Pass max_depth>1 to ALSO get engine-backed extensions: a stopped line continued by the color's engine-best moves until it rejoins prep several plies on — use this when a line looks unfinished but actually transposes into prep a few moves later.
