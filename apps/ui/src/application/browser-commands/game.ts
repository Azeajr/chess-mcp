...
export const gameCommands = {
  analyze_game: async (args, context) => {
    const records = await analyzeMainline(
      pgnFor(args, context.currentPgn),
  ...
  batch_review: async (args, context) => {
    const result = await batchReviewOperation(
      ...
      {
        groupBy:
          (args.group_by as "eco" | "color" | undefined) ??
          toolDefault("batch_review", "group_by", "eco"),
        username: args.username as string | undefined,
        maxGames:
          (args.max_games as number | undefined) ?? toolDefault("batch_review", "max_games", 100),
  ...
  lichess_games: async (args, context) => {
    const games = await context.lichessGames(
      ...
      (args.include_pgn as boolean | undefined) ??
        toolDefault("lichess_games", "include_pgn", false),
      context.signal,
    );
    throwIfAborted(context.signal);
    return games === null
  ...
  repertoire_vs_history: async (args, context) => {
    const platform =
      ...
      toolDefault("repertoire_vs_history", "platform", "lichess");
      ...
      platform === "chesscom"
        ...
        : await context.lichessGames(
            ...
            (args.max_games as number | undefined) ??
              toolDefault("repertoire_vs_history", "max_games", 30),
            undefined,
            true,
            context.signal,
          );
    ...
    return games === null
      ? { error: "fetch_failed", reason: "offline or unknown user" }
      : repertoireHistoryResult(context.currentTree(), context.currentColor(), games);
  },
