  ...
  return (
    <details class="rep-section strategic-fit-transfer">
      ...
      <div class="strategic-fit-transfer-actions">
        ...
        <Show
          when={intentState().status === "running"}
          fallback={
            <button
              class="fix-btn"
      ...
      <Show when={sidecarState().error}>
        <div class="empty" role="alert">
          {sidecarState().error}
        </div>
      </Show>
      <Show when={intentState().error}>
        <div class="empty" role="alert">
          {intentState().error}
        </div>
      </Show>
      <Show when={intentState().progress}>
        {(progress) => (
          <div class="scan-progress" role="status">
            {progress().detail ?? "Generating intent PGN"}{" "}
      ...
      <Show when={strategicFitSidecarImportPreview()}>
        {(preview) => (
          <section
            class="strategic-fit-import-preview"
            aria-label="Strategic Fit metadata import preview"
          >
            <h4>Import preview</h4>
            <dl>
              <dt>Source document</dt>
              <dd>{preview().source_document_id}</dd>
              <dt>Current document</dt>
              <dd>{preview().target_document_id}</dd>
              <dt>Profile</dt>
              <dd>
                {preview().profile.changed
            ...
            <ul class="strategic-fit-import-counts">
              <For each={Object.entries(collectionLabels)}>
                {([key, label]) => {
                  const summary = () => preview().collections[key as keyof typeof collectionLabels];
                  return (
                    <li>
                      <strong>{label}:</strong> {summary().added.length} add,{" "}
                      {summary().replaced.length} replace, {summary().preserved.length} preserve
                      <Show when={summary().incoming_stale.length > 0}>
                        {" "}
                        · {summary().incoming_stale.length} incoming stale
                      </Show>
                    </li>
                  );
                }}
            ...
            <div class="scope-note">
              Incoming records replace matching durable identities; unmatched current records are
              preserved. The incoming profile replaces the current profile only when this preview is
              confirmed.
            </div>
            <details class="strategic-fit-import-identities">
                  ...
                  return (
                    <Show
                      when={
                        ...
                        summary().incoming_stale.length > 0
                      }
                    >
                      <div>
                        <strong>{label}</strong>
                        <Show when={summary().added.length > 0}>
                          <div>Add: {summary().added.join(", ")}</div>
                        </Show>
                        <Show when={summary().replaced.length > 0}>
                          <div>Replace: {summary().replaced.join(", ")}</div>
                        </Show>
                        <Show when={summary().incoming_stale.length > 0}>
                          <div>Remain stale: {summary().incoming_stale.join(", ")}</div>
                        </Show>
                      </div>
                    </Show>
                  );
            ...
            <div class="scope-note">
              Resulting stale records:{" "}
              {Object.values(preview().resulting_stale).reduce(
            ...
            <Show when={Object.values(preview().resulting_stale).some((ids) => ids.length > 0)}>
              <details class="strategic-fit-import-identities">
                ...
                <For each={Object.entries(preview().resulting_stale)}>
                  {([key, ids]) => (
                    <Show when={ids.length > 0}>
                      <div>
                        <strong>{key.replace(/_/g, " ")}:</strong> {ids.join(", ")}
                      </div>
                    </Show>
                  )}
                </For>
