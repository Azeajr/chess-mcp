  ...
  return (
    ...
    >
      <div class="strategic-fit-analysis-lifecycle-copy" aria-live="polite">
        ...
        <Show when={isActive(state().status) && state().progress}>
          {(progress) => (
            <div class="strategic-fit-analysis-progress">
              <progress
                aria-label="Strategic Fit analysis progress"
                value={progress().total === undefined ? undefined : progress().done}
                max={progress().total}
              />
              <span>{progressPercent() === null ? "Analysis in progress" : `${progressPercent()}% complete`}</span>
            </div>
          )}
        </Show>
