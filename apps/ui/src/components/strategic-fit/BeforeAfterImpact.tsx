...
const number = (value: number | null, unit = "") =>
  value === null ? "Unavailable" : `${Number.isInteger(value) ? value : value.toFixed(3)}${unit}`;
...
const evaluation = (quality: ReplacementObjectiveQuality, repertoireColor: "white" | "black") => ({
  repertoire:
    quality.repertoire_pov_mate_in !== null
      ? `Mate ${quality.repertoire_pov_mate_in}`
      : quality.repertoire_pov_evaluation_cp === null
        ? "Unavailable"
        : `${quality.repertoire_pov_evaluation_cp} cp`,
  white:
    quality.white_pov_mate_in !== null
      ? `Mate ${quality.white_pov_mate_in}`
      : quality.white_pov_evaluation_cp === null
        ? "Unavailable"
        : `${quality.white_pov_evaluation_cp} cp`,
...
export function buildBeforeAfterImpact(preview: ReplacementChangeSetPreview) {
  ...
  return {
    tree: treeRows(preview).map(([label, before, after]) => ({
      label,
      before,
      after,
      delta: after - before,
    ...
    training: {
      ...
      delta:
        preview.strategic_score_before.training_cost === null ||
        preview.strategic_score_after.training_cost === null
          ? null
          : preview.strategic_score_after.training_cost -
            preview.strategic_score_before.training_cost,
    },
...
function ScoreImpact(props: {
  ...
  const rows = () =>
    [
      ...
      [
        "Expected coverage",
        props.before.expected_opponent_coverage,
        props.after.expected_opponent_coverage,
        "",
      ],
      ["Popularity", props.before.popularity, props.after.popularity, ""],
      [
        "Homogenization cost",
        props.before.homogenization_cost,
        props.after.homogenization_cost,
        "",
      ],
    ...
    <section aria-labelledby="replacement-impact-scores-title">
      ...
      <div class="replacement-impact-table-wrap">
        <table class="replacement-impact-table">
          <caption>
            Canonical before, after, and display deltas. Missing Phase 8 evidence remains
            unavailable.
          </caption>
          <thead>
            <tr>
          ...
          <tbody>
            <For each={rows()}>
              {(row) => (
                <tr
                  data-impact-state={
                    row[1] === null || row[2] === null ? "unavailable" : "available"
                  }
                >
                  <th scope="row">{row[0]}</th>
                  <td>{number(row[1], row[3])}</td>
                  <td>{number(row[2], row[3])}</td>
                  <td>{delta(row[1], row[2], row[3])}</td>
                </tr>
              )}
            </For>
          </tbody>
      ...
      <dl class="replacement-impact-theory">
        <div>
          <dt>Theory nodes before</dt>
          <dd>{number(props.after.theory_nodes_before)}</dd>
        </div>
        <div>
          <dt>Theory nodes after</dt>
          <dd>{number(props.after.theory_nodes_after)}</dd>
        </div>
        <div>
          <dt>Theory added</dt>
          <dd>{number(props.after.theory_nodes_added)}</dd>
        </div>
        <div>
          <dt>Theory removed</dt>
          <dd>{number(props.after.theory_nodes_removed)}</dd>
        </div>
      </dl>
    </section>
  );
  ...
  return (
    <tr data-impact-state={props.metric.state}>
      <th scope="row">
        ...
        <small>{props.metric.reason ?? "Canonical Phase 8 metric evidence."}</small>
      </th>
...
export default function BeforeAfterImpact(props: {
  ...
  const beforeEval = () =>
    evaluation(props.preview.objective_quality_before, props.repertoireColor);
  ...
  return (
    <section class="replacement-before-after" aria-labelledby="replacement-before-after-title">
      <header>
        ...
        <p>
          Values come from immutable Phase 8 change-set and safety evidence. No score, safety check,
          or frontier is recalculated by this view.
        </p>
      </header>
      ...
      <section aria-labelledby="replacement-tree-impact-title">
        ...
        <div class="replacement-impact-table-wrap">
          <table class="replacement-impact-table">
            ...
            <tbody>
              <For each={treeRows(props.preview)}>
                {(row) => (
                  <tr>
                    <th scope="row">{row[0]}</th>
                    <td>{row[1]}</td>
        ...
        <ul class="replacement-change-paths" aria-label="Affected descendant paths">
          <For
            each={props.preview.affected_paths}
            fallback={<li data-evidence-state="empty">No affected descendant paths.</li>}
          >
            {(path) => (
              <li>
      ...
      <section aria-labelledby="replacement-evaluation-impact-title">
        ...
        <dl class="replacement-impact-evaluation">
          <div>
            <dt>{beforeEval().owner} repertoire POV before</dt>
            <dd>{beforeEval().repertoire}</dd>
          </div>
          <div>
            <dt>{afterEval().owner} repertoire POV after</dt>
            <dd>{afterEval().repertoire}</dd>
          </div>
          <div>
            <dt>{afterEval().owner} repertoire POV cp delta</dt>
            <dd>
              {delta(
                props.preview.objective_quality_before.repertoire_pov_evaluation_cp,
                props.preview.objective_quality_after.repertoire_pov_evaluation_cp,
                " cp",
              )}
            </dd>
          </div>
          <div>
            <dt>White-POV engine transport before</dt>
            <dd>{beforeEval().white}</dd>
          </div>
          <div>
            <dt>White-POV engine transport after</dt>
            <dd>{afterEval().white}</dd>
          </div>
          <div>
            <dt>White-POV transport cp delta</dt>
            <dd>
              {delta(
                props.preview.objective_quality_before.white_pov_evaluation_cp,
                props.preview.objective_quality_after.white_pov_evaluation_cp,
                " cp",
              )}
            </dd>
          </div>
          <div>
            <dt>Canonical verdict after</dt>
            <dd>{props.preview.objective_quality_after.repertoire_pov_verdict}</dd>
          </div>
          <div>
            <dt>Engine evidence state</dt>
            <dd>{props.preview.objective_quality_after.state}</dd>
          </div>
        </dl>
      </section>
      ...
      <section aria-labelledby="replacement-coverage-impact-title">
        ...
        <dl class="replacement-impact-evaluation">
          <div>
            <dt>Weighted coverage before</dt>
            <dd>{percent(coverage().popularity_weighted_before)}</dd>
          </div>
          <div>
            <dt>Weighted coverage after</dt>
            <dd>{percent(coverage().popularity_weighted_after)}</dd>
          </div>
          <div>
            <dt>Coverage delta</dt>
            <dd>{percent(coverage().popularity_weighted_delta)}</dd>
          </div>
          <div>
            <dt>Required replies before</dt>
            <dd>{coverage().required_reply_count_before}</dd>
          </div>
          <div>
            <dt>Required replies after</dt>
            <dd>{coverage().required_reply_count_after}</dd>
          </div>
          <div>
        ...
        <div class="replacement-impact-replies">
          <section>
            ...
            <ul>
              <For
                each={coverage().newly_covered_replies}
                fallback={<li data-evidence-state="empty">None.</li>}
              >
                {(reply) => (
                  <li data-evidence-state={reply.state}>
                    <strong>{reply.san ?? "SAN unavailable"}</strong>
                    <code>
                      {reply.position_id} · {reply.decision_id ?? "decision unavailable"}
                    </code>
                    <span>
                      {reply.forcing ? "Forcing reply" : "Non-forcing reply"} ·{" "}
                      {percent(reply.expected_frequency)}
                    </span>
                    <small>
                      {reply.source_san_paths.map((path) => path.join(" ")).join(" | ") ||
                        reply.reason}
                    </small>
                  </li>
                )}
          ...
          <section>
            ...
            <ul>
              <For
                each={coverage().newly_uncovered_replies}
                fallback={<li data-evidence-state="empty">None.</li>}
              >
                {(reply) => (
                  <li data-evidence-state={reply.state}>
      ...
      <ScoreImpact
        before={props.preview.strategic_score_before}
        after={props.preview.strategic_score_after}
      />
      ...
      <section aria-labelledby="replacement-metric-impact-title">
        ...
        <div class="replacement-impact-table-wrap">
          <table class="replacement-impact-table">
            ...
            <tbody>
              <For
                each={coverage().affected_metrics}
                fallback={
                  <tr data-impact-state="unavailable">
                    <th scope="row">Metrics unavailable</th>
                    <td colSpan={3}>No canonical affected-metric evidence.</td>
                  </tr>
                }
              >
      ...
      <section aria-labelledby="replacement-safety-checks-title">
        ...
        <ul class="replacement-safety-checks">
          <For
            each={props.safety.safety_checks}
            fallback={<li data-check-status="unavailable">Safety checks unavailable.</li>}
          >
            {(check) => (
              <li data-check-status={check.status}>
                ...
                <Show when={check.risk_ids.length}>
