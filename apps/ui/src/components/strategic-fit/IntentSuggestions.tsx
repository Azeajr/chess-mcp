...
const pathLabel = (path: readonly string[]) => path.length === 0
  ? "Starting position"
  : path.join(" ");
        ...
        <div class="strategic-fit-intent-list">
          <For each={suggestions()}>{(suggestion) => (
            <article data-intent-disposition={suggestion.disposition ?? "pending"}>
              ...
              <div class="strategic-fit-intent-path"><span>Path</span> {pathLabel(suggestion.source_san_path)}</div>
              <blockquote>“{suggestion.source_comment}”</blockquote>
              <div class="strategic-fit-intent-match">Matched “{suggestion.source_match}”</div>
              <Show when={suggestion.disposition === null} fallback={(
                ...
                <div class="strategic-fit-intent-actions">
                  <button
                    type="button"
                    onClick={() => decideStrategicFitIntentComment(suggestion.suggestion_id, "confirmed")}
                  >Confirm intent</button>
                  <button
                    type="button"
                    onClick={() => decideStrategicFitIntentComment(suggestion.suggestion_id, "rejected")}
                  >Dismiss</button>
                </div>
              </Show>
            </article>
