...
function sourceStateRows() {
  ...
  const engine = result.engine_generation.source_results.map((entry) => ({
    ...
    detail:
      entry.reason ??
      `${entry.accepted_item_count} candidates retained at depth ${entry.reached_depth ?? "unavailable"}`,
  }));
...
export default function ReplacementLab() {
  ...
  const selectedRow = () =>
    comparisonRows().find((row) => row.candidate_id === selectedCandidateId()) ?? null;
      ...
      >
        ...
        <main class="replacement-lab-body">
          <section class="replacement-lab-context" aria-labelledby="replacement-lab-context-title">
            ...
            <p>
              <strong>{state().finding?.plain_language_category}</strong>
            </p>
            ...
            <dl>
              <div>
                <dt>Document</dt>
                <dd>
                  <code>{state().identity?.document_id}</code>
                </dd>
              </div>
              <div>
                <dt>Request</dt>
                <dd>
                  <code>{state().identity?.request_id}</code>
                </dd>
              </div>
              <div>
                <dt>Report</dt>
                <dd>
                  <code>{state().identity?.report_id}</code>
                </dd>
              </div>
              <div>
                <dt>Finding</dt>
                <dd>
                  <code>{state().identity?.finding_id}</code>
                </dd>
              </div>
              <div>
                <dt>Semantic finding</dt>
                <dd>
                  <code>{state().identity?.semantic_finding_id}</code>
                </dd>
              </div>
              <div>
                <dt>Document revision</dt>
                <dd>{state().identity?.repertoire_revision}</dd>
              </div>
              <div>
                <dt>Report revision</dt>
                <dd>
                  <code>{state().identity?.report_repertoire_revision}</code>
                </dd>
              </div>
              <div>
                <dt>Pivot</dt>
                <dd>
                  <code>{state().selected_pivot_decision_id ?? "Not confirmed"}</code>
                </dd>
              </div>
              <div>
                <dt>Profile</dt>
                <dd>
                  <code>{state().identity?.profile_identity}</code>
                </dd>
              </div>
              <div>
                <dt>Settings</dt>
                <dd>
                  <code>{state().identity?.settings_identity}</code>
                </dd>
              </div>
              <div>
                <dt>Repertoire owner</dt>
                <dd>{state().identity?.repertoire_color === "black" ? "Black" : "White"}</dd>
              </div>
            </dl>
            <p
              class="replacement-lab-pov"
              data-repertoire-color={state().identity?.repertoire_color}
            >
              User verdicts use {state().identity?.repertoire_color === "black" ? "Black" : "White"}{" "}
              repertoire POV. Engine cp/mate transport remains explicitly White POV.
            </p>
          </section>
            ...
            <section class="replacement-lab-alert" role="alert" data-replacement-lab-unavailable>
              <h3>
                {state().status === "stale"
                  ...
                  : "No replacement action is supported"}
              </h3>
          ...
          <Show when={state().actionability?.actionable === true}>
              ...
              <Show
                ...
                fallback={
                  <div class="replacement-lab-alert" role="alert">
                    No supported repertoire-owned pivot is available. Candidate generation is
                    blocked.
                  </div>
                }
              >
                  ...
                  <For each={pivotOptions()}>
                    {(pivot) => (
                      <label>
                        ...
                        <span>
                          <strong>
                            {pivot.san} · ply {pivot.ply}
                          </strong>
                          <small>
                            <code>{pivot.decision_id}</code> · repertoire-owned
                          </small>
                        </span>
                      </label>
                    )}
            ...
            <section
              class="replacement-lab-controls"
              aria-labelledby="replacement-lab-controls-title"
            >
              <h3 id="replacement-lab-controls-title">2. Candidate sources and engine</h3>
              <fieldset disabled={state().status === "running"}>
                <legend>Candidate sources</legend>
                <For each={REPLACEMENT_LAB_SUPPORTED_SOURCES}>
                  {(kind) => (
                    <label>
                      <input
                        type="checkbox"
                        ...
                        onInput={(event) =>
                          replacementLab.setSource(kind, event.currentTarget.checked)
                        }
                      />
                      <span>
                        <strong>{SOURCE_LABELS[kind]}</strong>
                        <small>Canonical Phase 8 producer</small>
                      </span>
                    </label>
                  )}
                </For>
                <For each={REPLACEMENT_LAB_UNAVAILABLE_SOURCES}>
                  {(kind) => (
                    <label data-source-unavailable="true">
                      <input type="checkbox" disabled />
                      <span>
                        <strong>{SOURCE_LABELS[kind]}</strong>
                        <small>Unavailable: no canonical Phase 8 seed producer</small>
                      </span>
                    </label>
                  )}
                </For>
              ...
              <label class="replacement-lab-depth">
                ...
                <input
                  ...
                  disabled={
                    state().status === "running" ||
                    !state().controls.sources.includes("engine-multipv")
                  }
              ...
              <details>
                ...
                <dl>
                  <div>
                    ...
                    <dd>{state().controls.maximum_candidates}</dd>
                  </div>
                  <div>
                    <dt>Subtree nodes / candidate</dt>
                    <dd>{state().controls.maximum_subtree_nodes_per_candidate}</dd>
                  </div>
                  <div>
                    <dt>Engine positions</dt>
                    <dd>{state().controls.maximum_engine_positions}</dd>
                  </div>
                  <div>
                    <dt>Explorer queries</dt>
                    <dd>{state().controls.maximum_explorer_queries}</dd>
                  </div>
                  <div>
                    <dt>Strategic horizon</dt>
                    <dd>ply {state().controls.strategic_horizon_ply}</dd>
                  </div>
                </dl>
              </details>
              <div class="replacement-lab-run-actions">
                ...
                <Show when={["cancelled", "failed", "partial"].includes(state().status)}>
                  <button
                    type="button"
                    disabled={!state().pivot_confirmed}
                    onClick={() => void replacementLab.retry()}
                  >
                    Retry generation
                  </button>
                </Show>
          ...
          <Show when={state().status === "running" && state().progress}>
            {(progress) => (
              <section
                class="replacement-lab-progress"
                aria-labelledby="replacement-lab-progress-title"
                aria-live="polite"
              >
          ...
          <Show when={state().result}>
            <section
              class="replacement-lab-results"
              aria-labelledby="replacement-lab-results-title"
            >
              <header>
                <h3 id="replacement-lab-results-title">3. Generated candidate previews</h3>
                <p>
                  Compare canonical Phase 8 evidence. Selection inspects one stable candidate
                  identity only; it never recommends, accepts, applies, or mutates.
                </p>
              </header>
                ...
                <Show
                  ...
                  fallback={
                    <div class="replacement-candidate-unavailable" role="status">
                      Select any chart point or candidate row to inspect exact subtree, axes,
                      identities, provenance, and risks. No candidate is preselected or labeled
                      best.
                    </div>
                  }
                >
