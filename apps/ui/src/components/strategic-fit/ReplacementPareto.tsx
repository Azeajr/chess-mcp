...
function normalized(row: CandidateComparisonRow, axisId: string): number | null {
  ...
  return axis?.state === "available" && axis.normalized_score !== null
...
export function buildReplacementParetoPoints(
  ...
  const key = (point: ReplacementParetoPoint) =>
    point.available
...
function memoryRadius(point: ReplacementParetoPoint): number {
  return point.memory_burden === null ? 0 : 2 + Math.min(1, Math.max(0, point.memory_burden)) * 4;
}
    ...
    <section class="replacement-pareto" aria-labelledby="replacement-pareto-title">
      ...
      >
        <desc id="replacement-pareto-description">
          Candidate tradeoff chart with keyboard-selectable points. Candidate table below is
          complete accessible equivalent.
        </desc>
        <line x1="38" y1="174" x2="334" y2="174" />
        <line x1="38" y1="174" x2="38" y2="28" />
        <text x="185" y="210" text-anchor="middle">
          Repertoire loss from engine best (0–300+ cp)
        </text>
        <text x="12" y="104" text-anchor="middle" transform="rotate(-90 12 104)">
          Strategic familiarity
        </text>
        <text x="40" y="190">
          Unavailable coordinates
        </text>
        <For each={drawn().items}>
          {(point, index) => {
            ...
            return (
              ...
              >
                ...
                <text class="replacement-pareto-point-label" x="15" y="-10">
                  {point.san}
                </text>
              </g>
            );
          ...
          {(point) => (
            <li data-pareto-status={point.status}>
              ...
              >
                ...
                <span>
                  {point.status}; {point.objective}; memory {point.memory}
                </span>
              </button>
            </li>
          )}
