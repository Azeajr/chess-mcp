...
export default function CohortEditor(props: {
  ...
  const routeOptions = () =>
    props.report.cohorts
      ...
      .sort(
        (left, right) =>
          left.cohortId.localeCompare(right.cohortId) || left.routeId.localeCompare(right.routeId),
      );
  ...
  const resetOptions = () =>
    [
      ...strategicFitMetadata()
        .cohort_overrides.filter((entry) => entry.record_state === "active")
        .map((entry) => ({
          value: `override:${entry.override_id}`,
          label: `${entry.kind === "merge" ? "Merge" : "Split"} · ${entry.route_ids.length} route(s)`,
        })),
      ...strategicFitMetadata()
        .exclusions.filter((entry) => entry.record_state === "active")
        .map((entry) => ({
          value: `override:${entry.override_id}`,
          label: `Subtree exclusion · ${entry.decision_ids?.length ?? 0} decision(s), ${entry.route_ids?.length ?? 0} route(s)`,
        })),
      ...strategicFitMetadata()
        .cohort_labels.filter((entry) => entry.record_state === "active")
        .map((entry) => ({
          value: `rename:${entry.label_id}`,
          label: `Name · ${entry.display_name}`,
        })),
  ...
  const toggle = (
    ...
    setter(
      selected ? [...new Set([...values, id])].sort() : values.filter((value) => value !== id),
    );
    cancelStrategicFitCohortAdjustment();
  };
  ...
  return (
    ...
    >
      ...
      <form onSubmit={requestPreview}>
              ...
              {(option) => (
                <label>
                  ...
                  <span>
                    ...
                    <small>Cohort {option.cohortId}</small>
                  </span>
                </label>
              )}
            </For>
        ...
        <Show when={mode() === "exclude"}>
          <fieldset class="strategic-fit-cohort-options">
            ...
            <Show
              when={decisionOptions().length > 0}
              fallback={
                <p data-impact-state="unavailable">
                  Decision references are unavailable for this report.
                </p>
              }
            >
        ...
        <Show when={mode() === "rename"}>
          <label class="strategic-fit-cohort-field">
            ...
            >
              <For each={props.report.cohorts}>
                {(cohort) => <option value={cohort.cohort_id}>{cohort.cohort_id}</option>}
              </For>
            </select>
          </label>
          <label class="strategic-fit-cohort-field">
        ...
        <button type="submit" disabled={strategicFitCohortAdjustment().status === "previewing"}>
          {strategicFitCohortAdjustment().status === "previewing"
            ? "Calculating preview…"
            : "Preview adjustment"}
        </button>
      </form>
      ...
      <Show when={strategicFitCohortAdjustment().preview}>
        {(preview) => (
          <section
            class="strategic-fit-cohort-preview"
            aria-labelledby={`cohort-preview-${preview().preview_id}`}
          >
            ...
            <div class="strategic-fit-cohort-preview-actions">
              <button type="button" onClick={confirm}>
                Confirm and analyze again
              </button>
              <button type="button" onClick={cancelStrategicFitCohortAdjustment}>
                Cancel preview
              </button>
            </div>
          </section>
