...
import ConceptComparison, { buildConceptComparisonPresentation } from "./ConceptComparison";
...
export function buildObjectivePresentation(
  ...
  return {
    ...
    loss_from_best:
      objective.loss_from_best_cp === null
        ? "Unavailable"
        : `${formatNumber(objective.loss_from_best_cp, 0)} cp`,
    engine_depth:
      objective.engine_depth === null ? "Unavailable" : formatNumber(objective.engine_depth, 0),
    engine_lines:
      objective.engine_lines === null ? "Unavailable" : formatNumber(objective.engine_lines, 0),
    database_performance:
      objective.database_performance === null
        ? "Unavailable"
...
export function buildEvidencePresentation(
  ...
  for (const dimension of finding.evidence.dimensions) {
    if (dimension.typical_value === null || dimension.affected_value === null) {
      limitations.push(
        `One side of ${dimension.dimension_id} is unavailable; it is not shown as zero.`,
      );
    }
  }
  ...
  if (basis.analysis_window === null)
    limitations.push("A comparable analysis window is unavailable.");
  ...
  for (const issueId of finding.evidence.data_quality_issue_ids) {
    ...
    limitations.push(
      issue?.message ?? "The report records a data-quality limitation for this comparison.",
    );
  }
  for (const entry of sources) {
    if (entry.source.state === "available") continue;
    limitations.push(
      entry.source.reason ?? `${entry.kind_label} is ${entry.state_label.toLowerCase()}.`,
    );
  }
...
export default function EvidencePanel(props: {
  ...
  const presentation = () =>
    buildEvidencePresentation(props.finding, props.preflightIssues, props.repertoireColor);
