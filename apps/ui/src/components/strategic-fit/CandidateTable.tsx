...
export function formatCanonicalAxisValue(
  ...
  const formatted = contribution.unit === "fraction" || contribution.unit === "score"
...
export function buildCandidateComparisonRows(
  ...
  return scoring.candidates.map((candidate) => {
    ...
    return {
      ...
      loss_from_best: candidate.objective_quality.repertoire_pov_loss_from_best_cp === null
        ? "Unavailable"
        : `${number(candidate.objective_quality.repertoire_pov_loss_from_best_cp)} cp`,
      engine_detail: candidate.objective_quality.engine_depth === null
...
export function resolveCandidateSelection(
  ...
  return candidateId !== null && rows.some((row) => row.candidate_id === candidateId) ? candidateId : null;
}
  ...
  return (
    <div class="replacement-candidate-table-wrap">
      <table class="replacement-candidate-table">
        <caption>
          Candidate comparison. Tradeoffs stay separate; Pareto status never means one aggregate best candidate.
        </caption>
        <thead>
          <tr>
            ...
            >
              <th scope="row" data-label="Candidate">
                <button
                  ...
                  aria-controls={props.selectedCandidateId === row.candidate_id
              ...
              <td data-label="Repertoire evaluation">
                ...
                <small>Loss {row.loss_from_best}; {row.engine_detail}</small>
              </td>
              <td data-label="Familiarity" data-axis-state={axis(row, "strategic-familiarity").state}>
                {axis(row, "strategic-familiarity").value}
              </td>
              <td data-label="Memory burden" data-axis-state={axis(row, "memorization-burden").state}>
              ...
              <td data-label="Risk">
                ...
                <small>{row.candidate.expansion.unresolved_risks.length} unresolved expansion risks</small>
              </td>
            </tr>
          )}</For>
        </tbody>
...
function routeSan(row: CandidateComparisonRow, edgeIds: readonly string[]): string {
  ...
  return edgeIds.map((edgeId) => edges.get(edgeId)?.san ?? `[Unavailable edge ${edgeId}]`).join(" ");
}
...
export function buildCandidateSubtreeRoutes(row: CandidateComparisonRow): readonly CandidateSubtreeRoutePresentation[] {
  return row.candidate.expansion.subtree?.routes.map((route) => ({
    route_id: route.route_id,
    san: routeSan(row, route.edge_ids) || "Root position",
    termination: route.termination,
    expected_frequency: route.expected_opponent_frequency === null
      ? "Unavailable"
  ...
  return (
    ...
    >
      ...
      <section aria-labelledby={`replacement-candidate-identity-${props.row.candidate_id}`}>
        <h5 id={`replacement-candidate-identity-${props.row.candidate_id}`}>Immutable identity and versions</h5>
        <dl class="replacement-candidate-identity">
          <div><dt>Request</dt><dd><code>{props.row.request_id}</code></dd></div>
          <div><dt>Report</dt><dd><code>{props.row.report_id}</code></dd></div>
          <div><dt>Finding</dt><dd><code>{props.row.finding_id}</code></dd></div>
          <div><dt>Semantic finding</dt><dd><code>{props.row.semantic_finding_id}</code></dd></div>
          <div><dt>Cohort</dt><dd><code>{props.row.cohort_id}</code></dd></div>
          <div><dt>Revision</dt><dd><code>{props.row.repertoire_revision}</code></dd></div>
          <div><dt>Pivot</dt><dd><code>{props.row.pivot_id ?? "Unavailable"}</code></dd></div>
          <div><dt>Schema</dt><dd><code>{props.row.schema_version}</code></dd></div>
          <div><dt>Analysis</dt><dd><code>{props.row.analysis_version}</code></dd></div>
          <div><dt>Replacement schema</dt><dd><code>{props.row.replacement_schema_version}</code></dd></div>
          <div><dt>Stage-only preview</dt><dd>{props.row.preview_status}</dd></div>
        </dl>
      </section>

      <section aria-labelledby={`replacement-candidate-evaluation-${props.row.candidate_id}`}>
        <h5 id={`replacement-candidate-evaluation-${props.row.candidate_id}`}>Evaluation and Pareto evidence</h5>
        <p>
        ...
        <p>
          <strong>White-POV engine transport:</strong> {props.row.white_pov_transport}. {props.row.engine_detail}.
        </p>
        <p>{props.row.pareto_reason}</p>
        <p>
          Active canonical axes: {props.row.active_pareto_axes.length > 0
            ? props.row.active_pareto_axes.join(", ")
            : "Unavailable"}.
        </p>
        <Show when={props.row.dominated_by_candidate_ids.length > 0}>
          <p>Exact dominators: <code>{props.row.dominated_by_candidate_ids.join(", ")}</code>.</p>
        </Show>
      </section>

      <section aria-labelledby={`replacement-candidate-axes-${props.row.candidate_id}`}>
        <h5 id={`replacement-candidate-axes-${props.row.candidate_id}`}>Canonical strategic axes</h5>
        <dl class="replacement-candidate-axes">
          <For each={props.row.axes}>{(item) => (
            <div data-axis={item.axis} data-axis-state={item.state}>
              ...
              <dd>
                ...
                provenance {item.provenance_source_ids.length > 0 ? item.provenance_source_ids.join(", ") : "Unavailable"}.
              </dd>
              <For each={item.provenance}>{(source) => (
                <dd>
                  <code>{source.source_id}</code> · {source.kind} · {source.state} · version {source.version ?? "Unavailable"} ·
      ...
      <section aria-labelledby={`replacement-candidate-concepts-${props.row.candidate_id}`}>
        <h5 id={`replacement-candidate-concepts-${props.row.candidate_id}`}>Concepts, transpositions, and risk</h5>
        <dl class="replacement-candidate-identity">
          <div>
            <dt>New concepts</dt>
            <dd>{props.row.concept_ids.length > 0 ? props.row.concept_ids.join(", ") : "None introduced in canonical evidence"}</dd>
          </div>
          <div>
            <dt>Transpositions</dt>
            <dd>{props.row.transposition_position_ids.length > 0
              ? props.row.transposition_position_ids.join(", ")
              : "None recorded in canonical evidence"}</dd>
          </div>
          <div>
            <dt>Safety state</dt>
            <dd>{props.row.safety === null
              ? "Unavailable"
              : `${props.row.safety.status}: ${props.row.safety.explanation}`}</dd>
          </div>
          <div>
        ...
        <Show when={props.row.candidate.expansion.unresolved_risks.length > 0}>
          <ul class="replacement-candidate-risk-list">
            <For each={props.row.candidate.expansion.unresolved_risks}>{(risk) => (
              <li data-risk-status={risk.status}>
                <strong>{risk.kind} · {risk.status}</strong>
        ...
        <Show when={props.row.safety?.safety_checks.length}>
          <ul class="replacement-candidate-risk-list" aria-label="Canonical safety checks">
            <For each={props.row.safety!.safety_checks}>{(check) => (
              <li data-risk-status={check.status}>
                <strong>{check.kind} · {check.status}</strong>
      ...
      <section aria-labelledby={`replacement-candidate-subtree-${props.row.candidate_id}`}>
        <h5 id={`replacement-candidate-subtree-${props.row.candidate_id}`}>Complete proposed subtree</h5>
        <Show when={subtree()} fallback={(
          <div class="replacement-candidate-unavailable" role="status">
            <strong>Subtree unavailable</strong>
            <p>{props.row.candidate.reason ?? "Canonical expansion returned no proposed subtree."}</p>
          </div>
        )}>
          {(tree) => (
            <>
              <p>
                ...
                {tree().edges.length} edges · {tree().routes.length} routes · horizon ply {tree().strategic_horizon_ply}.
              </p>
              <p>
                Important replies {tree().covered_important_reply_count}/{tree().important_reply_count}; forcing replies{" "}
                {tree().covered_forcing_reply_count}/{tree().forcing_reply_count}.
              </p>
              <Show when={tree().truncation_reasons.length > 0}>
                <p>Truncation: {tree().truncation_reasons.join(", ")}.</p>
              </Show>
              <ol class="replacement-subtree-routes">
                <For each={buildCandidateSubtreeRoutes(props.row)}>{(route) => (
                  <li data-route-termination={route.termination}>
                    <strong>{route.san}</strong>
                    <span>{route.termination}; expected frequency {route.expected_frequency}</span>
                  ...
                  <For each={tree().nodes}>{(node) => (
                    <li>
                      ...
                      <Show when={node.transposition_target_position_id}>
                        {(target) => <> · transposes to <code>{target()}</code></>}
                      </Show>
      ...
      <details class="replacement-candidate-provenance">
        ...
        <Show when={provenance().length > 0} fallback={<p>Provenance unavailable.</p>}>
          <ul>
            <For each={provenance()}>{(source) => (
              <li>
                <code>{source.source_id}</code> · {source.kind} · {source.status} · provider {source.provider ?? "Unavailable"} ·
                version {source.version ?? "Unavailable"} · snapshot {source.snapshot ?? "Unavailable"}
                <Show when={source.reason}>{(reason) => <p>{reason()}</p>}</Show>
                <For each={source.provenance}>{(nested) => (
                  <small>
                    <code>{nested.source_id}</code> · {nested.kind} · {nested.state} · version {nested.version ?? "Unavailable"} ·
                    snapshot {nested.snapshot ?? "Unavailable"}
        ...
        <Show when={props.row.safety?.error_code}>
          {(errorCode) => <p><code>{errorCode()}</code>: {props.row.safety!.explanation}</p>}
        </Show>
        <Show when={props.row.preview_error_code}>
          {(errorCode) => <p><code>{errorCode()}</code>: {props.row.preview_explanation ?? "Preview unavailable."}</p>}
        </Show>
        <Show
          when={props.row.candidate.expansion.evidence_item_results.some((item) => item.error_code !== null) ||
            props.row.candidate.expansion.omissions.length > 0}
          fallback={<p>No candidate-specific structured errors or omissions.</p>}
        >
          <ul>
            <For each={props.row.candidate.expansion.evidence_item_results.filter((item) => item.error_code !== null)}>{(item) => (
              <li><code>{item.error_code}</code> · {item.status} · {item.explanation}</li>
            )}</For>
            <For each={props.row.candidate.expansion.omissions}>{(item) => (
              <li><code>{item.omission_id}</code> · {item.reason} · {item.explanation}</li>
            )}</For>
          </ul>
        </Show>
      </details>
