    ...
    <Show when={recoverDialogOpen()}>
      ...
      >
        ...
        <Show when={snapshots().length > 0} fallback={<p>No recovery snapshots yet.</p>}>
          <div class="recover-list" aria-label="Recovery snapshots">
            <For each={snapshots()}>
              {(snapshot) => (
                  ...
                  >
                    <label>
                      ...
                      <span>
                        ...
                        <small>
                          {new Date(snapshot.savedAt).toLocaleString()} · {snapshot.moveCount} moves
          ...
          <Show when={preview()}>
            <pre class="recover-preview" aria-label="Snapshot PGN preview">
              {preview()}
            </pre>
          </Show>
          <Show when={error()}>
