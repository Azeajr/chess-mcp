...
const dependencies = [
  ...
  ...(item.prerequisites ?? []).map(
    (foundation) => `${foundation} (foundation): ${status(state.foundations?.[foundation])}`,
  ),
];
const gates = item.blockingGates.map((gate) => `${gate}: ${status(state.gates[gate])}`);
const ready =
  dependencies.every((entry) => entry.endsWith(": completed")) &&
  gates.every((entry) => entry.endsWith(": resolved"));
