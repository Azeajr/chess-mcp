...
const c2only = async (fen) =>
  fen.includes("PPP1PPPP/RNBQKBNR")
    ? [{ uci: "c2c4", cp: 30, mate: null }] // d4 Nf6: g1 knight still home
    : fen.includes("RNBQKB1R b")
      ? [{ uci: "e7e6", cp: 15, mate: null }] // after Nf3 (black to move)
      : [];
...
const e1Stub = async (fen, _mpv, depth) =>
  depth != null
    ? [{ uci: "a7a6", cp: 99, mate: null }] // deep-confirm call (any position) → force a value
    : fen.includes("5n2/8/3P4")
      ? [
          { uci: "c2c4", cp: 10, mate: null },
          { uci: "g1f3", cp: 20, mate: null },
...
const cmStub = async (fen) =>
  fen.includes("4P3")
    ? [{ uci: "e7e5", cp: 20, mate: null, depth: 10, pv: [] }] // after 1.e4 (Black to move)
