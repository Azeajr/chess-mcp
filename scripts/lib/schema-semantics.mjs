...
export function schemaSemanticDifferences(actual, expected, path = "$") {
  ...
  if (!actual || typeof actual !== "object" || Array.isArray(actual))
    return [`${path}: actual schema is not an object`];
  if (!expected || typeof expected !== "object" || Array.isArray(expected))
    return [`${path}: expected schema is not an object`];

  for (const key of SEMANTIC_KEYS) {
    if (stable(actual[key]) !== stable(expected[key]))
      differences.push(`${pointer(path, key)}: ${stable(actual[key])} != ${stable(expected[key])}`);
  }

  const actualRequired = [...(actual.required ?? [])].sort();
  const expectedRequired = [...(expected.required ?? [])].sort();
  if (stable(actualRequired) !== stable(expectedRequired))
    differences.push(
      `${pointer(path, "required")}: ${stable(actualRequired)} != ${stable(expectedRequired)}`,
    );
  ...
  if (stable(actualKeys) !== stable(expectedKeys))
    differences.push(
      `${pointer(path, "properties")}: ${stable(actualKeys)} != ${stable(expectedKeys)}`,
    );
  for (const key of expectedKeys.filter((key) => key in actualProperties))
    differences.push(
      ...schemaSemanticDifferences(
  ...
  if ("items" in expected || "items" in actual) {
    if (!("items" in actual) || !("items" in expected))
      differences.push(`${pointer(path, "items")}: one schema omits array items`);
    else
      differences.push(
        ...schemaSemanticDifferences(actual.items, expected.items, pointer(path, "items")),
      );
  }
  return differences;
}
