...
function budgetChecks(record, budgets) {
  ...
  for (const scale of record.scales ?? []) {
    ...
    checks.push(check(
      "determinism", scale.id, scale.determinism?.indexed_matches_cold === true,
      scale.determinism?.indexed_matches_cold, true, "boolean",
      "an indexed scan returns byte-identical findings to an unindexed one",
    ));
    checks.push(check(
      "incremental-determinism", scale.id, scale.determinism?.incremental_matches_cold === true,
      scale.determinism?.incremental_matches_cold, true, "boolean",
      "an incremental scan after an edit returns what a cold scan of the edited tree returns",
    ));
    checks.push(check(
      "fixture-determinism", scale.id, scale.fixture?.regenerated_digest === scale.fixture?.digest,
      scale.fixture?.regenerated_digest, scale.fixture?.digest, "digest",
    ...
    if (scaleBudgets.cold_reference_multiple !== null) {
      ...
      checks.push(check(
        "cold-scan", scale.id, scan.cold_ms <= budget, round(scan.cold_ms), round(budget), "ms",
    ...
    checks.push(check(
      "warm-scan", scale.id, scan.warm_ms <= warmBudget, round(scan.warm_ms), round(warmBudget), "ms",
    ...
    checks.push(check(
      "incremental-scan", scale.id, scan.incremental_ms <= incrementalBudget,
      round(scan.incremental_ms), round(incrementalBudget), "ms",
    ...
    checks.push(check(
      "graph-rebuild", scale.id, scan.graph_rebuild_ms <= graphBudget,
      round(scan.graph_rebuild_ms), round(graphBudget), "ms",
    ...
    checks.push(check(
      "cancellation-latency", scale.id, scale.cancellation.worst_phase_ms <= latencyBudget,
      round(scale.cancellation.worst_phase_ms), round(latencyBudget), "ms",
      "worst case a mid-phase cancellation waits for the longest phase to finish",
    ));
    checks.push(check(
      "cancellation-stop", scale.id,
      scale.cancellation.observed_ms <= budgets.cancellation_observed_ms,
      round(scale.cancellation.observed_ms), budgets.cancellation_observed_ms, "ms",
      "an observed cancellation stops the scan instead of finishing it",
    ));

    for (const operation of scale.main_thread ?? []) {
      checks.push(check(
        `frame:${operation.id}`, scale.id, operation.worst_ms <= budgets.frame_ms,
        round(operation.worst_ms), round(budgets.frame_ms), "ms",
    ...
    for (const bound of scale.bounds ?? []) {
      checks.push(check(
        `bound:${bound.id}`, scale.id, bound.actual <= bound.limit, bound.actual, bound.limit,
        bound.unit, bound.detail,
      ));
    }

    checks.push(check(
      "peak-heap", scale.id, scale.memory.peak_heap_mb <= scaleBudgets.peak_heap_mb,
      round(scale.memory.peak_heap_mb), scaleBudgets.peak_heap_mb, "MB",
...
function regressionChecks(record, baseline, tolerance) {
  ...
  for (const scale of record.scales ?? []) {
    ...
    for (const metric of ["cold_ms", "warm_ms", "incremental_ms"]) {
      ...
      checks.push(check(
        `regression:${metric}`, scale.id, now <= budget, round(now), round(budget), "ms",
 ...
 */
export function syntheticStrategicFitBenchmarkRecord(manifest = { schema_version: "0", analysis_version: "0" }) {
  return {
    ...
    scales: [{
      id: "standard",
      fixture: { nodes: 1_000, leaves: 437, digest: "0000000000000000", regenerated_digest: "0000000000000000" },
      findings: 437,
      ...
      bounds: [{ id: "index-entries", actual: 250, limit: 512, unit: "entries", detail: "the index stays inside its bound" }],
    }],
  };
}
...
export const STRATEGIC_FIT_BENCHMARK_SELF_CHECKS = Object.freeze([
  ["a cold scan that outgrew the reference workload", (record) => { record.scales[0].scan.cold_ms = 5_000; }],
  ["a warm scan that lost its index", (record) => { record.scales[0].scan.warm_ms = 99; }],
  ["an incremental scan slower than a cold one", (record) => { record.scales[0].scan.incremental_ms = 140; }],
  ["a graph rebuild that grew into a full scan", (record) => { record.scales[0].scan.graph_rebuild_ms = 80; }],
  ["a main-thread operation over one frame", (record) => { record.scales[0].main_thread[0].worst_ms = 40; }],
  ["a cache bound exceeded", (record) => { record.scales[0].bounds[0].actual = 513; }],
  ["a peak heap over the ceiling", (record) => { record.scales[0].memory.peak_heap_mb = 9_999; }],
  ["a cancellation the scan did not observe", (record) => { record.scales[0].cancellation.observed_ms = 500; }],
  ["a non-deterministic indexed scan", (record) => { record.scales[0].determinism.indexed_matches_cold = false; }],
  ["a fixture that generated differently", (record) => { record.scales[0].fixture.regenerated_digest = "ffffffffffffffff"; }],
]);
...
export function runStrategicFitBenchmarkSelfCheck() {
  ...
  if (!passing.ok) {
    failures.push(`a within-budget record must pass: ${passing.failures.map((entry) => entry.id).join(", ")}`);
  }
  for (const [description, perturb] of STRATEGIC_FIT_BENCHMARK_SELF_CHECKS) {
    const record = syntheticStrategicFitBenchmarkRecord();
    perturb(record);
    if (evaluateStrategicFitBenchmark(record).ok) failures.push(`the gate must reject ${description}`);
  }
  ...
  const incomparable = syntheticStrategicFitBenchmarkRecord({ schema_version: "9", analysis_version: "9" });
...
function line(entry) {
  ...
  const actual = entry.unit === "boolean" || entry.unit === "digest"
    ? String(entry.actual)
    : `${entry.actual} ${entry.unit}`;
  const budget = entry.unit === "boolean" || entry.unit === "digest"
    ? String(entry.budget)
