...
const models = (process.env.OPENROUTER_MODELS ?? "")
  ...
  .filter(Boolean);
const maxLiveRounds = Math.max(
  1,
  Math.min(4, Number(process.env.OPENROUTER_EVAL_MAX_ROUNDS ?? 2) || 2),
);

if (!apiKey || !models.length) {
  console.error(
    "Set OPENROUTER_API_KEY and comma-separated OPENROUTER_MODELS before running this credentialed verification.",
  );
  process.exit(2);
}
...
const cases = [
  {
    id: "ambiguous_repertoire",
    prompt: "What are the biggest problems here?",
    any: ["audit_repertoire_moves", "find_repertoire_gaps", "get_repertoire_coverage"],
  },
  { id: "audit", prompt: "Audit my prescribed repertoire moves.", all: ["audit_repertoire_moves"] },
  {
    id: "only_moves",
    prompt: "Find only moves in my repertoire and prepare a drill deck.",
    all: ["find_only_moves"],
  },
  {
    id: "structures",
    prompt: "Find Carlsbad structures in my repertoire.",
    all: ["find_structures"],
  },
  {
    id: "opponent",
    prompt: "Prepare my repertoire against Lichess opponent alice.",
    all: ["prep_vs_opponent"],
  },
  {
    id: "annotated_repertoire",
    prompt: "Export an annotated version of this repertoire.",
    all: ["export_annotated_repertoire"],
  },
  { id: "position", prompt: "Evaluate this position.", all: ["evaluate_position"] },
  {
    id: "position_followup",
    prompt: "What about g4?",
    any: ["evaluate_position", "compare_moves"],
    preserve: true,
  },
  {
    id: "switch_repertoire",
    prompt: "Now audit this repertoire instead.",
    all: ["audit_repertoire_moves"],
    preserve: true,
  },
  {
    id: "switch_game",
    prompt: "I loaded a game. Review the current game and summarize the mistakes.",
    any: ["get_game_summary", "analyze_game"],
    preserve: true,
    document: "game",
  },
];
...
const fixtureFor = (name, args = {}) => {
  switch (name) {
    case "get_position":
      return { fen: startFen, turn: "white", legal_moves: ["g4", "e4", "d4"], color: "white" };
    case "get_legal_moves":
      return {
        fen: args.fen ?? startFen,
        moves: args.fen === afterG4 ? ["e5", "d5"] : ["g4", "e4", "d4"],
      };
    case "validate_line":
      return { ok: true, canonical: ["g4"], finalFen: afterG4 };
    case "evaluate_position":
      return {
        fen: afterG4,
        eval_pov: "white",
        lines: [{ san: "e5", cp: 12, mate: null, depth: 14 }],
      };
    case "audit_repertoire_moves":
      return { color: "white", positions_scanned: 4, moves_audited: 4, findings: [] };
    case "find_only_moves":
      return { color: "white", positions_scanned: 4, only_moves_found: 0, findings: [], lines: [] };
    case "find_structures":
      return {
        color: "white",
        leaves_total: 3,
        total_matches: 1,
        matches: [{ path: ["d4", "d5", "c4", "e6"], structure: "Carlsbad" }],
      };
    ...
    case "export_annotated_repertoire":
      return {
        kind: "artifact",
        artifact_id: "eval-artifact",
        format: "pgn",
        name: "repertoire-annotated.pgn",
        bytes: 128,
      };
    case "get_game_summary":
      return {
        ...
        black: { accuracy_pct: 88 },
        worst_moves: [],
      };
    case "analyze_game":
      return { total_moves: 6, moves: [] };
    case "get_document_pgn":
      return { revision: 1, pgn: game.actions.toPgn() };
    case "get_selected_subtree":
      return { selected_path: [], lines: [["e4", "e5", "Nf3", "Nc6"]], truncated: false };
...
chat.setChatTransportForTesting(async (options) => {
  ...
  if (active.satisfied || active.rounds.length >= maxLiveRounds)
    return { content: "Selection captured for verification.", toolCalls: [] };
...
report.passed = report.models.every((modelResult) =>
  modelResult.cases.every((testCase) => testCase.passed),
);
