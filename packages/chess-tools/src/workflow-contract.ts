...
export const WORKFLOW_CONTRACTS: Record<WorkflowFamily, WorkflowContract> = {
  ...
  repertoire: {
    ...
    steps: [
      ...
      step("Extend and connect", "Use coverage for dangling lines and stub reconnection. Keep legacy one-move replacement suggestions compatible; for Strategic Fit V2 compare complete candidate subtrees, coverage, safety, provenance, and atomic change-set previews. Browser previews are staged for explicit acceptance; MCP previews do not provide archive storage or undo and return a new handle only after an explicit edit.", ["get_repertoire_coverage", "suggest_complementary_lines", "suggest_replacement_line"]),
