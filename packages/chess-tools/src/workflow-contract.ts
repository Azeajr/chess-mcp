...
export const WORKFLOW_CONTRACTS: Record<WorkflowFamily, WorkflowContract> = {
  ...
  repertoire: {
    ...
    steps: [
      ...
      step("Analyze strategic fit", "Run the versioned Strategic Fit report with an explicit profile or the labeled inferred default. Optional population and personal-history sources must retain availability and sample provenance; never treat sparse history as zero frequency. Review expected-weight findings and their evidence; never treat difference, uncertainty, forced diversity, or intentional diversity as a defect.", ["analyze_repertoire_congruence", "get_structural_profile"]),
