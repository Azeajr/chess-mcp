...
import {
  calculateStrategicFamiliarityAdjustedCoverage,
} from "./metrics.js";
...
export type ReplacementSafetyResultStatus =
  (typeof REPLACEMENT_SAFETY_RESULT_STATUSES)[number];
...
function unavailableMetricInputProvenance(kind: "personal-history" | "manual-weight" | "finding-context"):
...
function sameIdentity(
  value: Pick<ReplacementCandidateScoringResult,
  ...
  return {
    ...
    profile: {
      ...
      preferences: {
        ...
        preferred_tactical_character: sortedUnique(result.profile.preferences.preferred_tactical_character),
      },
    },
...
function boundaryFailure(
  ...
  if (!sameVersions(request) || !sameVersions(scoring) || !sameIdentity(scoring, request)) {
    return {
      ...
      explanation: "Task 8.6 scoring identities, color, revision, or schema versions do not match the current request.",
    };
  }
  if (sourceGraph.repertoire_color !== request.repertoire_color ||
    stableJson(sourceGraph) !== stableJson(scoring.context.graph)) {
    return {
      status: "stale",
      error: "source-graph-mismatch",
      explanation: "Source tree does not reproduce the canonical Task 8.6 graph and cannot be simulated safely.",
    };
  }
  if ((scoring.status !== "complete" && scoring.status !== "partial") || scoring.error_code !== null ||
    !scoring.source_graph_unchanged || !scoring.source_context_unchanged ||
    !scoring.expansion_unchanged || !scoring.inputs_unchanged ||
    (recomputed.status !== "complete" && recomputed.status !== "partial") ||
    recomputed.error_code !== null || stableJson(scoring) !== stableJson(recomputed)) {
    return {
      status: "stale",
      error: "scoring-not-current",
      explanation: "Only a current deterministic Task 8.6 result with preserved Task 8.3-8.6 evidence can be simulated.",
    };
  }
  ...
  for (const action of actions) {
    ...
    if (typeof rawAction.candidate_id !== "string" ||
...
function candidateProvenance(candidate: ReplacementScoredCandidate): StrategicFitSourceProvenance[] {
...
function candidateBoundaryError(
  ...
  if (!sameVersions(candidate) || !sameIdentity(candidate, request) ||
    candidate.repertoire_color !== candidate.expansion.seed.mover_color) {
    return {
      error: "candidate-identity-mismatch",
      explanation: "Candidate identity, version, pivot, or repertoire ownership does not match its Task 8.6 boundary.",
    };
  }
  if (candidate.pareto.status === "unscored" || candidate.state === "unavailable" ||
    candidate.trajectory_report === null || candidate.concept_dictionary === null) {
    return {
      error: "candidate-unscored",
      explanation: "Unscored Task 8.6 candidates remain inspectable but cannot masquerade as safety simulations.",
    };
  }
  if (candidate.expansion.status !== "complete" || candidate.expansion.subtree?.status !== "complete") {
    return {
      error: "candidate-expansion-incomplete",
      explanation: "Truncated, blocked, stale, cancelled, illegal, or unavailable Task 8.5 work cannot be simulated safely.",
    };
  }
  return null;
}
...
function simulateTree(
  ...
  if (action === "replace") {
    for (const decisionPath of [...decisionPaths].sort((left, right) =>
...
function popularityByDecision(
  ...
  const evidenceById = new Map((evidence?.decision_weights ?? []).map((item) => [item.decision_id, item.weight]));
  ...
  for (const values of grouped.values()) {
    ...
    if (weighted.some((item) => item.weight === undefined || !Number.isFinite(item.weight) || item.weight! < 0)) continue;
...
function requirementsFor(
  ...
  for (const decision of before.decisions) {
    ...
    addRequirement(requirements, {
      ...
      reasons: ["Current population-weighted opponent reply remains required while its semantic position is reachable."],
  ...
  const edgeByReply = new Map(expansion.subtree.edges
  ...
  for (const edge of expansion.subtree.edges) {
    ...
    addRequirement(requirements, {
      ...
      san: edge.san,
      frequencies: edge.expected_opponent_frequency === null ? [] : [edge.expected_opponent_frequency],
      forcing: edge.forcing,
      sourceSanPaths: edge.source_san_paths,
      reasons: [edge.forcing
  ...
  for (const item of expansion.evidence_item_results) {
    ...
    const matchedEdge = edgeByReply.get(`${item.position.position_id}${SEPARATOR}${item.canonical_san ?? item.input_san}`);
    addRequirement(requirements, {
      ...
      sourceSanPaths: [],
      reasons: [`Task 8.5 ${item.important ? "important" : "forcing"} reply evidence is ${item.status}.`],
...
function covered(graph: RepertoireGraph, reply: RequiredReply): boolean {
  if (reply.decisionId !== null && graph.decisions.some((decision) =>
    decision.decision_id === reply.decisionId && decision.owner === "opponent")) return true;
  return reply.san !== null && graph.decisions.some((decision) =>
    decision.from_position_id === reply.positionId && decision.owner === "opponent" && decision.san === reply.san
  );
}

function effect(reply: RequiredReply, reason: string): ReplacementCoverageReplyEffect {
  const known = reply.frequencies.filter((value) => Number.isFinite(value) && value >= 0 && value <= 1);
  const state: ReplacementCoverageEffectState = known.length === 0 ? "unavailable"
    : known.length === reply.frequencies.length ? "available" : "partial";
  return {
    analysis_version: STRATEGIC_FIT_ANALYSIS_VERSION,
    state,
    ...
    san: reply.san,
    expected_frequency: known.length === reply.frequencies.length && known.length > 0
...
function weightedCoverage(replies: readonly RequiredReply[], graph: RepertoireGraph): number | null {
  ...
  const coveredWeight = replies.reduce((sum, reply, index) =>
    sum + (covered(graph, reply) ? frequencies[index]! : 0), 0
  );
...
function duplicateBranchIds(
  ...
  const beforeRoutes = new Map(before.routes.map((route) => [route.route_id, route.source_route_count]));
  const duplicates = after.routes.filter((route) =>
  ...
  for (const route of before.routes) {
    ...
    if (before.decisions.some((decision) =>
      decision.from_position_id === fromPosition && decision.uci === candidateUci
...
function newTranspositionIds(before: RepertoireGraph, after: RepertoireGraph): string[] {
  ...
  return sortedUnique(after.transposition_links
...
function affectedMetrics(
  ...
  try {
    const knownWeights = new Map<string, { weight: number; provenance: readonly StrategicFitSourceProvenance[] }>();
    for (const item of scoring.context.popularity?.decision_weights ?? []) {
      knownWeights.set(item.decision_id, { weight: item.weight, provenance: item.provenance ?? [] });
    }
    const expansion = candidate.expansion as ReplacementCompleteCandidateExpansion;
    for (const edge of expansion.subtree.edges) {
    ...
    const decisionWeights = [...opponentByPosition.values()].flatMap((group) => group.flatMap((decision) => {
      const known = knownWeights.get(decision.decision_id);
      if (known) return [{ decision_id: decision.decision_id, weight: known.weight, provenance: known.provenance }];
    ...
    const marketState = popularityState === "complete" ? "available" as const
      : popularityState === "partial" ? "partial" as const : "unavailable" as const;
    const weights = calculateStrategicRouteWeights(after, {
      ...
      market: {
        ...
        provenance: mergeProvenance(
          scoring.context.popularity?.provenance ?? [],
          marketState === "unavailable" ? [unavailablePopularityProvenance("metric-weighting")] : [],
        ),
      },
      provenance: mergeProvenance([CORE_PROVENANCE], candidateProvenance(candidate), missingWeightSources),
    });
    ...
    if (missingWeightSources.length > 0 || marketState !== "available" || weights.state !== "complete") {
      afterMetric = {
        ...afterMetric,
        state: afterMetric.state === "unavailable" ? "unavailable" : "partial",
        reason: `${afterMetric.reason ?? "Canonical familiarity coverage was calculated."} ` +
          "Missing requested profile weighting sources or conditional-decision fallbacks keep the post-clone metric partial.",
        provenance: mergeProvenance(afterMetric.provenance, missingWeightSources,
          marketState === "unavailable" ? [unavailablePopularityProvenance("metric-weighting")] : []),
      };
    }
  } catch {
    afterMetric = {
      ...
      reason: "Canonical post-clone familiarity inputs could not be rebuilt from retained Task 8.6 evidence.",
  ...
  const state: ReplacementCoverageEffectState = before === null || afterValue === null
    ? "unavailable"
    : beforeMetric.state === "partial" || afterMetric.state === "partial" ? "partial" : "available";
  const familiarityEffect: ReplacementMetricEffect = {
    ...
    unit: beforeMetric.unit,
    reason: afterMetric.reason ?? "Canonical post-clone familiarity coverage was rebuilt from semantic graph, weighting, concept, and training evidence.",
    provenance: mergeProvenance(beforeMetric.provenance, afterMetric.provenance, candidateProvenance(candidate)),
  };
  ...
  const trainingEffect: ReplacementMetricEffect = {
    ...
    provenance: mergeProvenance(trainingBefore.provenance, candidateProvenance(candidate), [missingFinding]),
  };
...
function coverageEffects(
  ...
  const popularityState = scoring.context.popularity?.state;
  const popularityIncomplete = popularityState === undefined || popularityState === "partial" ||
    popularityState === "unavailable" || popularityState === "cancelled";
  const missingFrequency = replies.some((reply) => reply.frequencies.length === 0);
  const state: ReplacementCoverageEffectState = replies.length === 0 && !popularityIncomplete
    ? "available"
    : beforeCoverage === null || afterCoverage === null
      ? replies.some((reply) => reply.frequencies.length > 0) ? "partial" : "unavailable"
      : missingFrequency || popularityIncomplete ? "partial" : "available";
  const newlyUncovered = replies.filter((reply) => covered(before, reply) && !covered(after, reply))
    .map((reply) => effect(reply, reply.forcing
  ...
  const newlyCovered = replies.filter((reply) => !covered(before, reply) && covered(after, reply))
    .map((reply) => effect(reply, reply.forcing
  ...
  return {
    ...
    popularity_weighted_delta: beforeCoverage === null || afterCoverage === null
      ? null : round(afterCoverage - beforeCoverage),
...
function checksFor(
  ...
  const engineStatus: ReplacementSafetyCheck["status"] = objective.state === "unavailable" ||
      objective.repertoire_pov_verdict === "unverified"
    ? "unavailable"
    : objective.state === "partial" ? "warning"
      : objective.repertoire_pov_verdict === "outside-tolerance" ||
          objective.repertoire_pov_verdict === "forced-mate-against-repertoire"
        ? "blocked" : "passed";
  const coverageStatus: ReplacementSafetyCheck["status"] = uncovered.length > 0
    ? "blocked" : effects.state === "available" ? "passed"
      : effects.state === "partial" ? "warning" : "unavailable";
  const metricStatus: ReplacementSafetyCheck["status"] = metricStates.length > 0 &&
      metricStates.every((state) => state === "available")
    ? "passed" : metricStates.some((state) => state === "partial") ? "warning" : "unavailable";
  ...
  return [
    safetyCheck("legality", "passed", "Every simulated SAN route rebuilt into a legal canonical graph on a clone.", provenance),
    safetyCheck("engine-sanity", engineStatus,
      engineStatus === "passed" ? "Repertoire-POV objective evidence passes its configured verdict; White-POV transport remains separately labeled."
        : engineStatus === "blocked" ? "Repertoire-POV objective evidence is outside tolerance or forced mate against the repertoire."
        : "Complete repertoire-POV objective evidence is unavailable; engine safety is not passed.", objective.provenance),
    safetyCheck("coverage", coverageStatus,
      uncovered.length > 0 ? `${uncovered.length} required opponent replies become uncovered.`
        : effects.state === "available" ? "Popularity-weighted required-reply coverage is complete."
        : "Coverage evidence is missing or partial; known values remain explicit and the check is not passed.", effects.provenance),
    safetyCheck("gap-scan", uncovered.length > 0 ? "blocked" : effects.state === "available" ? "passed" : "unavailable",
      uncovered.length > 0 ? "Clone comparison creates required-reply gaps."
        : effects.state === "available" ? "No required-reply gap appears in the cloned semantic graph."
        : "Gap absence cannot be established from incomplete coverage evidence.", effects.provenance),
    safetyCheck("transpositions", effects.new_transposition_position_ids.length > 0 ? "warning" : "passed",
      effects.new_transposition_position_ids.length > 0
        ...
        : "No new canonical transposition position appears.", effects.provenance),
    safetyCheck("duplicates", effects.duplicate_branch_ids.length > 0 ? "warning" : "passed",
      effects.duplicate_branch_ids.length > 0
        ...
        : "No new semantic or editorial duplicate branch appears.", effects.provenance),
    safetyCheck("stale-revision", "passed", "Request, source graph, scoring context, and retained Task 8.3-8.6 evidence were revalidated as current.", provenance),
    safetyCheck("affected-cohort-preview", metricStatus,
      metricStatus === "passed" ? "Affected Strategic Fit metric before/after/delta evidence is complete."
...
function emptyCoverage(candidate: ReplacementScoredCandidate, reason: string): ReplacementCoverageEffects {
  return {
    ...versioned(),
    state: "unavailable",
...
function simulateCandidate(
  ...
  if (boundary) return unavailableCandidate(input.request, sourceGraph, candidate, action, boundary.error, boundary.explanation);
  const clone = simulateTree(input.source_tree, sourceGraph, candidate, action);
  if (!clone) return unavailableCandidate(input.request, sourceGraph, candidate, action, "simulation-failed",
  ...
  try {
    ...
    return unavailableCandidate(input.request, sourceGraph, candidate, action, "simulation-failed",
  ...
  const incomplete = candidate.state !== "available" || effects.state !== "available" ||
    checks.some((check) => check.kind === "engine-sanity" &&
      (check.status === "warning" || check.status === "unavailable"));
  const status: ReplacementSafetyCandidateStatus = blocked ? "blocked" : incomplete ? "partial" : "safe";
...
function baseResult(
  ...
  return {
    ...
    candidates: ordered,
    safe_candidate_ids: ordered.filter((candidate) => candidate.status === "safe").map((candidate) => candidate.candidate_id),
    partial_candidate_ids: ordered.filter((candidate) => candidate.status === "partial").map((candidate) => candidate.candidate_id),
    blocked_candidate_ids: ordered.filter((candidate) => candidate.status === "blocked").map((candidate) => candidate.candidate_id),
    unavailable_candidate_ids: ordered.filter((candidate) => candidate.status === "unavailable").map((candidate) => candidate.candidate_id),
    provenance: mergeProvenance([CORE_PROVENANCE], input.request.provenance, scoring.provenance,
...
export function simulateReplacementSafety(
  ...
  try {
    ...
    return baseResult(input, cloneJson(input.scoring), "stale", "scoring-not-current",
  ...
  if (failure) return baseResult(input, recomputed, failure.status, failure.error, failure.explanation, []);
  const candidates = recomputed.candidates.map((candidate) =>
    simulateCandidate(input, sourceGraph, recomputed, candidate)
  );
  const status: ReplacementSafetyResultStatus = candidates.length === 0 ? "unavailable"
    : candidates.some((candidate) => candidate.status === "blocked") ? "blocked"
      : candidates.every((candidate) => candidate.status === "safe") ? "complete"
        : candidates.every((candidate) => candidate.status === "unavailable") ? "unavailable" : "partial";
  return baseResult(input, recomputed, status, null,
    status === "complete" ? "Every current Task 8.6 candidate was safely simulated on an isolated clone."
      : status === "blocked" ? "At least one explicitly pruning replacement is blocked by required-reply coverage loss."
