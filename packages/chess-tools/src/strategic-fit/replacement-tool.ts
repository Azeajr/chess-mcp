...
import type { ReplacementSafetyCandidateAction, ReplacementSafetySimulationResult } from "./replacement-safety.js";
import {
...
export const REPLACEMENT_TOOL_V2_ITEM_STATUSES = ["previewed", "stale", "invalid", "blocked", "cancelled"] as const;
export type ReplacementToolV2ItemStatus = (typeof REPLACEMENT_TOOL_V2_ITEM_STATUSES)[number];

export const REPLACEMENT_TOOL_V2_RESULT_STATUSES = ["complete", "partial", "stale", "invalid", "cancelled"] as const;
...
function canonicalRequest(request: ReplacementRequest): ReplacementRequest {
  return {
    ...
    provenance: [...request.provenance].sort((left, right) => left.source_id.localeCompare(right.source_id)),
  };
}
...
function validProvenance(values: readonly StrategicFitSourceProvenance[]): boolean {
  return Array.isArray(values) && values.every((value) =>
    value !== null && typeof value === "object" && typeof value.source_id === "string" &&
    typeof value.kind === "string" && typeof value.state === "string");
}
...
function item(
  ...
  return {
    ...versioned(), candidate_id: candidateId, status, error_code: code, explanation,
    change_set: changeSet, preview, provenance: structuredClone(provenance),
  };
}
...
function boundary(input: ReplacementToolV2Input): ReplacementToolV2ErrorCode | null {
  const request = input.replacement_request;
  if (input.contract !== REPLACEMENT_TOOL_V2_CONTRACT || input.candidate_ids.length === 0) return "invalid-request";
  if (!validProvenance(request.provenance) || !validProvenance(input.safety.provenance) ||
    input.safety.candidates.some((candidate) => !validProvenance(candidate.provenance))) return "invalid-request";
  if (request.schema_version !== STRATEGIC_FIT_SCHEMA_VERSION ||
    request.analysis_version !== STRATEGIC_FIT_ANALYSIS_VERSION ||
    request.replacement_schema_version !== STRATEGIC_FIT_REPLACEMENT_SCHEMA_VERSION) return "version-mismatch";
  if (!same(input.finding, {
    report_id: request.report_id,
    finding_id: request.finding_id,
    semantic_finding_id: request.semantic_finding_id,
    cohort_id: request.cohort_id,
  ...
  if (!same([...input.sources].sort(), [...request.candidate_sources].sort())) return "source-mismatch";
  if (!same(input.budget, request.budget)) return "budget-mismatch";
  if (input.engine.depth !== request.budget.engine_depth || input.engine.multipv !== request.budget.engine_multipv) return "engine-mismatch";
  if (input.coverage.minimum_expected_opponent_coverage !== request.minimum_expected_opponent_coverage ||
    input.coverage.require_all_forcing_replies !== request.budget.include_all_forcing_replies) return "coverage-mismatch";
  if (!same(canonicalRequest(input.safety.request), canonicalRequest(request)) || input.safety.repertoire_revision !== request.repertoire_revision ||
    input.safety.request_id !== request.request_id || input.safety.report_id !== request.report_id ||
    input.safety.finding_id !== request.finding_id || input.safety.semantic_finding_id !== request.semantic_finding_id ||
    input.safety.cohort_id !== request.cohort_id || input.safety.repertoire_color !== request.repertoire_color) return "safety-mismatch";
  ...
  if (retention.size !== input.retention.length || input.retention.some((entry) => !selected.has(entry.candidate_id))) return "retention-mismatch";
  for (const candidateId of input.candidate_ids) {
    const simulated = input.safety.candidates.find((candidate) => candidate.candidate_id === candidateId);
    if (simulated?.action === "replace" && !retention.has(candidateId)) return "retention-mismatch";
  }
  for (const entry of input.retention) {
    const simulated = input.safety.candidates.find((candidate) => candidate.candidate_id === entry.candidate_id);
    if (!simulated || simulated.action !== entry.action ||
      (entry.action === "replace" && entry.prune_explicitly_confirmed !== true)) return "retention-mismatch";
  }
  return null;
}
...
export function produceReplacementToolV2Previews(
  ...
  if ((options.expected_repertoire_revision !== undefined &&
    ...
    return result(input, "stale", "safety-mismatch",
  ...
  try {
    ...
    return result(input, "invalid", "invalid-request",
      "Malformed retained replacement evidence could not be decoded.", []);
  }
  if (boundaryError) return result(input, boundaryError.includes("mismatch") ? "stale" : "invalid", boundaryError,
  ...
  if (cancelled()) return result(input, "cancelled", "cancelled", "Replacement preview generation was cancelled.", []);
  const retention = new Map(input.retention.map((entry) => [entry.candidate_id, entry]));
  const items: ReplacementToolV2Item[] = [];
  for (const candidateId of input.candidate_ids) {
    if (cancelled()) {
      items.push(item(candidateId, "cancelled", "cancelled", "Candidate preview was cancelled.", input.safety.provenance));
      continue;
    }
    const candidate = input.safety.candidates.find((entry) => entry.candidate_id === candidateId);
    if (!candidate) {
      items.push(item(candidateId, "invalid", "candidate-not-found", "Candidate is absent from current safety evidence.", input.safety.provenance));
      continue;
    }
    let constructed;
    try {
      constructed = constructReplacementChangeSet({
        ...
        promote_candidate_to_mainline: retention.get(candidateId)?.promote_candidate_to_mainline === true,
      });
    } catch {
      items.push(item(candidateId, "invalid", "preview-failed",
        "Malformed retained candidate evidence could not be decoded.", input.safety.provenance));
      continue;
    }
    if (!constructed.change_set) {
      items.push(item(candidateId, constructed.status === "stale" ? "stale" : "blocked",
        constructed.error_code, constructed.explanation, candidate.provenance));
      continue;
    }
    let preview;
    try {
      ...
      items.push(item(candidateId, "invalid", "preview-failed",
        "Malformed retained change-set evidence could not be decoded.", candidate.provenance, constructed.change_set));
      continue;
    }
    if (preview.status !== "success") {
      items.push(item(candidateId, preview.output.status === "stale" ? "stale" : "invalid",
        preview.output.failure.code as ReplacementChangeSetErrorCode, preview.output.failure.explanation,
        candidate.provenance, constructed.change_set));
      continue;
    }
    items.push(item(candidateId, "previewed", null,
      "Complete atomic change-set preview is ready for explicit host staging.", candidate.provenance,
      constructed.change_set, preview.output));
  }
  ...
  return result(input,
    wasCancelled && previewed === 0 ? "cancelled" : stale && previewed === 0 ? "stale" : previewed === items.length ? "complete" : "partial",
