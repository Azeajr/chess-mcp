...
function requireCompatibleInputs(
  ...
  if (
    !sameIds(
      taxonomy.routes.map((route) => route.route_id),
      routeIds,
    )
  ) {
  ...
  if (
    !sameIds(
      trajectories.trajectories.map((route) => route.route_id),
      routeIds,
    )
  ) {
  ...
  if (
    !sameIds(
      weights.routes.map((route) => route.route_id),
      routeIds,
    )
  ) {
...
function repertoireDecisionIds(
  ...
  return new Set(
    route.decision_ids.filter((decisionId) => decisions.get(decisionId)?.owner === "repertoire"),
  );
}
...
function routeContexts(
  ...
  const trajectoryByRoute = new Map(
    trajectories.trajectories.map((route) => [route.route_id, route]),
  );
  ...
  return graph.routes
    .map((route): RouteContext => {
      ...
      return {
        ...
        comparableCheckpointKinds: new Set(
          comparableSnapshots.map((snapshot) => snapshot.checkpoint.kind),
        ),
        stableSignalFamilies: new Set(
          comparableSnapshots.flatMap((snapshot) =>
            snapshot.signals
              .filter(
                (signal) =>
...
function transpositionPositions(graph: RepertoireGraph, routeIds: ReadonlySet<string>): string[] {
...
function routesTranspose(graph: RepertoireGraph, leftId: string, rightId: string): boolean {
  return graph.transposition_links.some(
    (link) => link.route_ids.includes(leftId) && link.route_ids.includes(rightId),
  );
}
...
function sameNarrowOpeningScope(left: RouteContext, right: RouteContext): boolean {
  ...
  if (leftSystem !== null || rightSystem !== null)
    return leftSystem !== null && leftSystem === rightSystem;
...
function comparablePair(graph: RepertoireGraph, left: RouteContext, right: RouteContext): boolean {
  ...
  const sharedPlayerScope =
    intersection(left.repertoireDecisionIds, right.repertoireDecisionIds).length > 0;
  return sharedAncestor && sharedPlayerScope && trajectoriesShareNeighborhood(left, right);
}

function inferredRouteGroups(
  graph: RepertoireGraph,
  ...
  for (const override of overrides) {
    ...
    for (const routeId of selectedRoutes) {
      if (!routeIds.has(routeId))
        throw new Error(`strategic_fit_cohorts_unknown_route: ${routeId}`);
...
function applyStructuralOverrides(
  ...
  for (const override of overrides
    ...
    if (touched.length < 2)
      throw new Error(`strategic_fit_cohorts_invalid_merge: ${override.override_id}`);
    const extracted = new Set<string>();
...
function exclusionState(
  ...
  for (const override of overrides.filter((value) => value.kind === "exclude")) {
    ...
    for (const route of graph.routes) {
      if (
        (override.decision_ids ?? []).some((decisionId) => route.decision_ids.includes(decisionId))
      ) {
...
function commonOpeningScope(contexts: readonly RouteContext[]): string[] {
  return commonValues(
    contexts.map((context) => new Set(context.taxonomy.path.map((node) => node.taxonomy_id))),
  ).sort(compareStrings);
}
...
function normalizedCohortWeights(contexts: readonly RouteContext[]): {
  ...
  if (contexts.length === 0)
    return { routeWeights: [], effectiveSampleSize: 0, independentRouteCount: 0 };
  ...
  const routeWeightById = new Map(
    routeWeights.map((weight) => [weight.route_id, weight.normalized_weight]),
  );
...
function makeCohort(
  ...
  const decisionScopeIds = sortedUnique(
    baselineContexts.flatMap((context) =>
      context.actionableDecisionId ? [context.actionableDecisionId] : [],
    ),
  );
  const comparableEvidenceContexts = includedContexts.filter(
    (context) =>
...
function makeContainers(
  ...
  const containers = [...identities.values()]
    .map((identity): StrategicOpeningContainer => {
...
function taxonomyProvenance(taxonomy: RepertoireOpeningTaxonomy): StrategicFitSourceProvenance {
  ...
  return {
    ...
    state:
      unknownCount === 0
        ? "available"
        : unknownCount === taxonomy.routes.length
          ? "unavailable"
          : "partial",
    version: taxonomy.taxonomy_version,
    snapshot: null,
    reason:
      unknownCount === 0 ? null : `${unknownCount} route(s) lack deterministic opening taxonomy.`,
  };
}
...
export function formStrategicCohorts(
  ...
  const insufficientEvidenceRouteCount = contexts.filter(
    (context) =>
      context.comparableCheckpointKinds.size === 0 || context.stableSignalFamilies.size === 0,
  ).length;
  ...
  return {
    ...
    data_quality: {
      ...
      complete_trajectory_route_count: contexts.filter(
        (context) => context.trajectory.state === "complete",
      ).length,
      incomplete_trajectory_route_count: contexts.filter(
        (context) => context.trajectory.state !== "complete",
      ).length,
