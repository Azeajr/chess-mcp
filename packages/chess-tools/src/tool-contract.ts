...
export type ToolInput = {
  properties: Readonly<Record<string, InputField>>;
  browserProperties?: Readonly<Record<string, InputField>>;
  mcpProperties?: Readonly<Record<string, InputField>>;
  required?: readonly string[];
  mcpRequired?: readonly string[];
};
...
const define = (
  ...
  defaults,
  result: {
    kind: capabilities.includes("action")
      ? "action"
    ...
    ...(name === "analyze_repertoire_congruence"
      ? {
          semantics:
            "Versioned Strategic Fit V2 report with immutable summary, findings, preflight, paging, and provenance.",
        }
      : {}),
    ...(name === "suggest_replacement_line"
      ? {
          semantics:
            "A complete revision-bound Strategic Fit V2 candidate/change-set preview envelope. No host silently applies a preview.",
    ...
    ...(name === "get_strategic_fit_report"
      ? {
          semantics:
            "Bounded conversation projection of an existing immutable report: identity, overview state, one compact finding page, or one finding with evidence and navigable paths. Never the full report, provenance, or any document artifact.",
        }
      : {}),
    ...(name === "propose_strategic_fit_profile"
      ? {
          semantics:
            "Staged, revision-bound profile proposal with the exact field-level diff against the current effective profile. Proposing changes nothing: the profile, the repertoire tree, and cached reports are untouched until the user accepts in the application.",
        }
      : {}),
    ...(name === "propose_strategic_fit_plan"
      ? {
          semantics:
            "Either the bounded deterministic evidence basis for one finding, or a staged plan card validated against it. Proposing changes nothing: training metadata, the resolution, and the repertoire tree are untouched until the user accepts in the application.",
        }
      : {}),
    ...(name === "propose_strategic_fit_portfolio"
      ? {
          semantics:
            "Constraints staged for confirmation with any contradictions named, a bounded Pareto portfolio whose options are existing candidates with their retained measurements and change sets, or one option staged through the existing change-review path. Nothing is selected automatically, nothing is applied, and no preference is persisted.",
        }
      : {}),
  },
  hostAdaptation: {
    browserInjects:
      name === "export_strategic_fit_metadata"
        ...
        : name === "export_strategic_fit_intent_pgn"
          ? [
              ...
              "stable document ID",
              "document revision",
              "normalized Strategic Fit metadata",
              "current Strategic Fit report",
            ]
          : name === "analyze_repertoire_congruence"
            ? [
                "current PGN",
                ...
                "document revision",
                "opening taxonomy",
                "optional explorer credentials",
                "optional fetched personal-game PGNs",
                "Strategic Fit Web Worker",
              ]
            : name === "suggest_replacement_line"
              ? [
                  "current GameTree",
                  ...
                  "stable document ID",
                  "document revision",
                  "browser engine/Worker and explorer boundaries",
                  "revision-bound staged change-set storage",
                ]
              : name === "get_strategic_fit_report"
                ? [
                    "document revision",
                    "bounded cached Strategic Fit report lookup by report identity",
                  ]
                : name === "propose_strategic_fit_profile"
                  ? [
                      "stable document ID",
                      "document revision",
                      "current effective Strategic Fit profile",
                      "Strategic Fit analysis-settings identity",
                      "session-only staged proposal storage",
                    ]
                  : name === "propose_strategic_fit_plan"
                    ? [
                        "stable document ID",
                        "document revision",
                        "current Strategic Fit report and finding",
                        "deterministic training record for that finding",
                        "session-only staged plan storage",
                        "the existing training writer used on acceptance",
                      ]
                    : name === "propose_strategic_fit_portfolio"
                      ? [
                          "stable document ID",
                          "document revision",
                          "current effective Strategic Fit profile",
                          "the open Replacement Lab's retained Task 8.6 scoring, Task 8.7 safety, and Task 8.8 change-set previews",
                          "session-only staged constraint and portfolio storage",
                          "the existing revision-bound change-review staging path used on selection",
                        ]
                      : input?.properties.repertoire_id
                        ? ["current GameTree", "repertoire color"]
                        : [
                            ...(input?.properties.fen && !(input.required ?? []).includes("fen")
                              ? ["current FEN"]
                              : []),
                            ...(input?.properties.pgn && !(input.required ?? []).includes("pgn")
                              ? ["current PGN"]
                              : []),
                          ],
    mcpInjects:
      name === "analyze_repertoire_congruence"
        ? [
            "repertoire handle lookup",
            "handle revision",
            "bounded opening taxonomy",
            "optional explorer credentials",
            "optional fetched personal-game PGNs",
          ]
        : name === "suggest_replacement_line"
          ? [
              "immutable repertoire handle lookup",
              "handle revision",
              "Node engine pool and optional explorer credentials",
              "explicit no-archive/no-undo limitation",
            ]
          : name === "get_strategic_fit_report"
            ? [
                "repertoire handle lookup",
                "handle revision",
                "bounded per-handle Strategic Fit report lookup by report identity",
              ]
            : input?.properties.repertoire_id
              ? ["repertoire handle lookup"]
              : [],
    ...(name === "get_position"
      ? { resultDifference: "browser adds current repertoire color" }
      : name === "modify_repertoire_line"
        ? {
            resultDifference:
              "MCP returns a clone-on-write handle; browser returns a non-mutating preview",
          }
        : name === "analyze_game"
          ? { resultDifference: "MCP supports the host-only verbose result projection" }
          : name === "analyze_repertoire_congruence"
            ? {
                resultDifference:
                  "Browser execution uses the dedicated Worker; MCP runs the deterministic analyzer in-process. Each host optionally collects bounded explorer evidence and fetched personal-game PGNs before that shared analyzer boundary.",
              }
            : name === "propose_strategic_fit_profile"
              ? {
                  resultDifference:
                    "Browser only. MCP keeps no document profile: an MCP session passes the confirmed profile explicitly with each analysis, so there is nothing there to stage, diff, or persist.",
                }
              : name === "propose_strategic_fit_plan"
                ? {
                    resultDifference:
                      "Browser only. Training records, resolutions, and drill artifacts are browser document state; an MCP session keeps none of them, so it has nothing to ground a plan card in and nothing to save it to.",
                  }
                : name === "propose_strategic_fit_portfolio"
                  ? {
                      resultDifference:
                        "Browser only. A portfolio option is one of the open Replacement Lab's retained candidates, and selecting it stages a change for revision-bound confirmation; an MCP session holds no lab result and no staging, archive, or undo, so it has nothing to build a portfolio from and nowhere to stage one. On MCP, `suggest_replacement_line` already returns the same previews as immutable results.",
                    }
                  : name === "get_strategic_fit_report"
                    ? {
                        resultDifference:
                          "Each host resolves the report identity in its own bounded cache — per handle on MCP, per document settings in the browser — and both project the same shared bounded views. A report that is not cached, or that belongs to another revision, fails closed rather than returning older evidence.",
                      }
                    : name === "suggest_replacement_line"
                      ? {
                          resultDifference:
                            "Browser V2 results are staged against the exact current document revision and require explicit acceptance. MCP V2 results are immutable previews only: no archive persistence or undo is available, and a new clone-on-write handle is returned only by an explicit repertoire edit call.",
                        }
                      : {}),
  },
  ...(input ? { input } : {}),
});
const string = (description?: string, maxLength?: number, pattern?: string): InputField => ({
  type: "string",
  ...(description ? { description } : {}),
  ...(maxLength == null ? {} : { maxLength }),
  ...(pattern == null ? {} : { pattern }),
});
const integer = (minimum?: number, maximum?: number): InputField => ({
  type: "integer",
  ...(minimum == null ? {} : { minimum }),
  ...(maximum == null ? {} : { maximum }),
});
const number = (minimum?: number, maximum?: number): InputField => ({
  type: "number",
  ...(minimum == null ? {} : { minimum }),
  ...(maximum == null ? {} : { maximum }),
});
const array = (items: InputField = string(), minItems?: number, maxItems?: number): InputField => ({
  type: "array",
  items,
...
const strategicFitIdList = (minimum: number | undefined = undefined, maximum = 500) =>
...
const strategicFitIntentPreferences = object({
  ...
  preferred_tactical_character: array(
    string("Lowercase term such as forcing, sharp, or quiet.", 32, "^[a-z][a-z0-9-]*$"),
    undefined,
    12,
  ),
  minimum_opponent_coverage: number(0, 1),
...
const strategicFitWeighting = object({
  ...
  route_weights: array(
    object({ route_id: strategicFitId(), weight: number(0, 1_000_000) }, ["route_id", "weight"]),
    undefined,
    500,
  ),
  decision_weights: array(
    object({ decision_id: strategicFitId(), weight: number(0, 1_000_000) }, [
      "decision_id",
      "weight",
    ]),
    undefined,
...
const explorerRecency = string(
  "Lichess: YYYY-MM; masters: YYYY",
  7,
  "^(?:\\d{4}|\\d{4}-(?:0[1-9]|1[0-2]))$",
);
...
export const TOOL_CONTRACTS = [
  define(
    ...
    BOTH,
    {},
    { properties: { fen: string() }, required: ["fen"] },
  ),
  define(
    "validate_pgn",
    "Validate a PGN; returns the game count.",
    ["game"],
    BOTH,
    {},
    { properties: { pgn: string() }, required: ["pgn"] },
  ),
  define(
    "validate_line",
    ...
    {},
    {
      properties: { fen: string("FEN; browser defaults to the current position"), moves: array() },
      required: ["moves"],
      mcpRequired: ["fen", "moves"],
    },
  ),
  define(
    "get_legal_moves",
    "Legal moves (SAN) at a FEN.",
    ["position"],
    BOTH,
    {},
    {
      properties: { fen: string("FEN; browser defaults to the current position") },
      mcpRequired: ["fen"],
    },
  ),
  define(
    "get_position",
    "Ground the current position with its normalised FEN and legal moves; the browser also includes current-document context.",
    ["position"],
    BOTH,
    {},
    {
      properties: { fen: string("FEN; browser uses its current position when omitted") },
      mcpRequired: ["fen"],
    },
  ),
  define(
    "evaluate_position",
    "Local Stockfish multi-line analysis with white-POV cp/mate scores.",
    ["position", "engine"],
    BOTH,
    { depth: 20, lines: 3 },
    {
      properties: {
        fen: string("FEN; browser defaults to the current position"),
        depth: integer(1, 30),
        lines: integer(1, 5),
      },
      mcpRequired: ["fen"],
  ...
  define(
    "compare_moves",
    "Rank candidate SAN moves by local Stockfish (mover POV); illegal moves are returned separately.",
    ["position", "engine"],
    BOTH,
    { depth: 20 },
    {
  ...
  define(
    "cloud_eval",
    "Lichess cloud evaluation (white-POV) for a FEN, or unavailable.",
    ["position", "network"],
    BOTH,
    {},
    {
  ...
  define(
    "tablebase_lookup",
    "Lichess tablebase result for a seven-piece-or-fewer FEN, or unavailable.",
    ["position", "network"],
    BOTH,
    {},
    {
  ...
  define(
    "position_popularity",
    "Lichess opening-explorer statistics for a configured game population, including move frequencies and white-POV results.",
    ["position", "network"],
    BOTH,
    { db: "lichess", top_moves: 12 },
    {
      properties: {
        fen: string("FEN; browser defaults to the current position"),
        ...explorerPopulationFilters,
        top_moves: integer(0, 30),
      },
      mcpRequired: ["fen"],
    },
  ),
  define(
    "identify_opening",
    "Name the deepest ECO opening reached by a PGN.",
    ["position", "game"],
    BOTH,
    {},
    {
      properties: { pgn: string("PGN; browser defaults to the current working line") },
      mcpRequired: ["pgn"],
    },
  ),
  define(
    ...
    ["repertoire", "engine"],
    BOTH,
    { depth: 20, limit: 20, popularity_db: "lichess" },
    {
      properties: {
  ...
  define(
    "suggest_gap_fills",
    "Build best-evaluation and best-fit repertoire lines for one uncovered opponent move.",
    ["repertoire", "engine"],
    BOTH,
    { depth: 20, limit: 4 },
    {
      properties: {
        ...
        depth: integer(1, 30),
        limit: integer(2, 10),
        target_plies: integer(2, 200),
      },
      required: ["variation_path", "uncovered_move"],
      mcpRequired: ["repertoire_id", "variation_path", "uncovered_move"],
    },
  ),
  define(
    "find_theory_depth",
    "Report where repertoire lines leave known opening theory using explorer game counts.",
    ["repertoire", "network"],
    BOTH,
    { db: "lichess", max_positions: 60 },
    {
      properties: {
        repertoire_id: string("MCP handle; browser injects the current document"),
        db: { type: "string", enum: ["lichess", "masters"] },
  ...
  define(
    ...
    "Find sound moves that transpose into another prepared line and shorten memorisation.",
    ["repertoire", "engine"],
    BOTH,
    { limit: 20, multipv: 4, cp_threshold: 50, depth: 20 },
    {
      properties: {
        repertoire_id: string("MCP handle; browser injects the current document"),
        limit: integer(1, 100),
        multipv: integer(1, 8),
        cp_threshold: integer(0, 500),
        max_loss_cp: integer(0, 1000),
        depth: integer(1, 30),
        movetime_ms: integer(50, 10000),
        budget: integer(1, 500),
        leaf_start: integer(0),
        leaf_count: integer(1, 200),
        confirm_depth: integer(1, 30),
      },
      mcpRequired: ["repertoire_id"],
    },
  ),
  define(
    ...
    "Report dangling lines and natural frontiers; optionally engine-check whether stubs reconnect.",
    ["repertoire", "engine"],
    BOTH,
    { limit: 20, connect_stubs: false, depth: 20 },
    {
      properties: {
        repertoire_id: string("MCP handle; browser injects the current document"),
        limit: integer(1, 100),
        connect_stubs: { type: "boolean" },
        depth: integer(1, 30),
      },
      mcpRequired: ["repertoire_id"],
    },
  ...
  define(
    ...
    "Return a repertoire-wide pawn-structure profile or one position selected by SAN path.",
    ["repertoire"],
    BOTH,
    {},
    {
  ...
  define(
    ...
    BOTH,
    {
      ...
      popularity_db: "lichess",
      popularity_max_positions: 60,
      personal_history_platform: "lichess",
      personal_history_max_games: 30,
      page_limit: 50,
    },
    {
      properties: {
  ...
  define(
    ...
    "Find NAG-marked side lines that can inflate repertoire analysis counts.",
    ["repertoire"],
    BOTH,
    { limit: 20 },
    {
  ...
  define(
    ...
    "Apply or preview a prune, add, or reorder edit by SAN path.",
    ["repertoire", "action"],
    BOTH,
    {},
    {
      ...
      mcpRequired: ["repertoire_id", "action", "path"],
    },
  ),
  define(
    "suggest_complementary_lines",
    "Suggest engine-sound moves ranked for structural fit or imbalance.",
    ["repertoire", "engine"],
    BOTH,
    { depth: 20, limit: 5 },
    {
      ...
      mcpRequired: ["repertoire_id", "fen"],
    },
  ),
  define(
    "suggest_replacement_line",
    "Validate retained Task 8.7 evidence into complete Strategic Fit V2 atomic change sets without silently applying them.",
    ["repertoire", "engine", "action"],
    BOTH,
    {},
    {
      properties: {
        ...
        replacement_request: openObject(
          "Complete canonical ReplacementRequest with finding, profile, source, budget, identity, version, and provenance inputs.",
          { request_id: strategicFitId() },
          ["request_id"],
        ),
        finding: openObject(
          "Revision-bound report/finding/semantic-finding/cohort identity.",
          { finding_id: strategicFitId() },
          ["finding_id"],
        ),
        pivot: openObject(
          "Automatic or explicit semantic pivot selection.",
          { kind: { type: "string", enum: ["automatic", "user-selected"] } },
          ["kind"],
        ),
        profile: openObject(
          "Exact Strategic Fit profile snapshot used by the request.",
          { mode: { type: "string", enum: ["familiar-plans", "balanced", "versatile", "custom"] } },
          ["mode"],
        ),
        sources: array(
        ...
        budget: openObject(
          "Exact bounded candidate, engine, explorer, subtree, and strategic-horizon budget.",
          {
            engine_depth: integer(1, 30),
            engine_multipv: integer(1, 10),
          },
          ["engine_depth", "engine_multipv"],
        ),
        engine: openObject(
          "Exact engine depth/multipv request and unavailable-evidence policy.",
          {
            depth: integer(1, 30),
            multipv: integer(1, 10),
            allow_unavailable_evidence: { type: "boolean" },
          },
          ["depth", "multipv", "allow_unavailable_evidence"],
        ),
        coverage: openObject(
          "Exact minimum coverage and forcing-reply policy.",
          {
        ...
        retention: array(
          openObject(
            "Per-candidate add-alternative or explicitly confirmed archive-before-prune replacement choice.",
            {
        ...
        safety: openObject(
          "Complete immutable Task 8.3-8.7 evidence envelope, including structured per-item errors and provenance.",
          { request_id: strategicFitId() },
          ["request_id"],
        ),
      },
  ...
  define(
    "analyze_game",
    "Per-move engine review of a game's mainline with centipawn loss and classification.",
    ["game", "engine"],
    BOTH,
    { depth: 20 },
    {
      properties: {
        pgn: string("PGN; browser defaults to the current working line"),
        depth: integer(1, 30),
      },
      mcpProperties: { verbose: { type: "boolean" } },
      mcpRequired: ["pgn"],
    },
  ),
  define(
    "get_game_summary",
    "Game-review summary with per-side counts, accuracy, and worst moves.",
    ["game", "engine"],
    BOTH,
    { depth: 20 },
    {
  ...
  define(
    ...
    "Annotate a game's mainline with move glyphs and best-move/evaluation comments.",
    ["game", "engine", "artifact"],
    BOTH,
    { depth: 20 },
    {
      properties: {
  ...
  define(
    "batch_review",
    "Analyze multiple games and aggregate results by opening or player color.",
    ["game", "engine"],
    BOTH,
    { group_by: "eco", max_games: 100, depth: 20 },
    {
      properties: {
        pgn: string(),
  ...
  define(
    ...
    ["game", "network"],
    BOTH,
    { max_games: 20, include_pgn: false },
    {
      properties: {
        username: string(),
        max_games: integer(1, 100),
  ...
  define(
    "chesscom_games",
    "Fetch games for a Chess.com user in a given month.",
    ["game", "network"],
    BOTH,
    { include_pgn: false },
    {
  ...
  define(
    ...
    "Compare a repertoire with a user's games and report all departures.",
    ["repertoire", "game", "network"],
    BOTH,
    { platform: "lichess", max_games: 30 },
    {
      properties: {
        repertoire_id: string("MCP handle; browser injects the current document"),
        username: string(),
      ...
      required: ["username"],
      mcpRequired: ["repertoire_id", "username"],
    },
  ),
  define(
    "audit_repertoire_moves",
    "Engine-check prescribed moves tree-wide and rank findings by centipawn loss.",
    ["repertoire", "engine"],
    BOTH,
    { depth: 20, min_cp_loss: 50, max_positions: 20, limit: 10 },
    {
      properties: {
        repertoire_id: string(),
        depth: integer(1, 30),
        min_cp_loss: integer(0),
        max_positions: integer(1, 60),
        limit: integer(1, 50),
      },
      mcpRequired: ["repertoire_id"],
  ...
  define(
    "find_only_moves",
    "Find positions where the best move clearly exceeds the second choice.",
    ["repertoire", "engine", "artifact"],
    BOTH,
    { depth: 20, min_margin: 100, max_positions: 300, limit: 25, lines_limit: 10 },
    {
      properties: {
        repertoire_id: string(),
        depth: integer(1, 30),
        min_margin: integer(0),
        max_positions: integer(1, 300),
        limit: integer(1, 100),
        lines_limit: integer(1, 50),
      },
      browserProperties: { export_deck: { type: "boolean" } },
      mcpProperties: { export_path: string() },
      mcpRequired: ["repertoire_id"],
    },
  ),
  define(
    "find_structures",
    "Search repertoire leaves by structure, center, themes, or color complex.",
    ["repertoire"],
    BOTH,
    { min_confidence: 0.6, limit: 30 },
    {
      properties: {
        repertoire_id: string(),
        structure: string(),
        min_confidence: { type: "number", minimum: 0, maximum: 1 },
        center: { type: "string", enum: ["tense", "locked", "open", "semi-open"] },
        themes: array({
          type: "string",
          enum: [
            "fianchetto_white",
        ...
        color_complex: { type: "string", enum: ["light", "dark"] },
        limit: integer(1, 100),
      },
      mcpRequired: ["repertoire_id"],
    },
  ...
  define(
    ...
    "Check whether pruning a shortcut line creates an uncovered gap.",
    ["repertoire", "engine"],
    MCP,
    { depth: 20 },
    {
      ...
      mcpRequired: ["repertoire_id", "line_path", "at_ply"],
    },
  ),
  define(
    "compare_shortcut_lines",
    "Compare shortcut candidates by engine quality and structural fit.",
    ["repertoire", "engine"],
    MCP,
    { depth: 20, eval_tiebreak_cp: 30 },
    {
      properties: {
        repertoire_id: string(),
        line_path: array(),
        ...
        joins_path: array(),
        depth: integer(1, 30),
        eval_tiebreak_cp: integer(0, 500),
      },
      mcpRequired: ["repertoire_id", "line_path", "at_ply", "joins_path"],
    },
  ),
  define(
    "inspect_shortcut",
    "Inspect one shortcut candidate for both line quality and coverage safety.",
    ["repertoire", "engine"],
    BROWSER,
    { depth: 20, max_positions: 12, eval_tiebreak_cp: 30 },
    {
      properties: {
        line_path: array(),
        at_ply: integer(0),
      ...
      required: ["line_path", "at_ply", "joins_path"],
    },
  ),
  define(
    "export_annotated_repertoire",
    "Run analyses and produce a cloned, annotated repertoire PGN.",
    ["repertoire", "engine", "artifact"],
    BOTH,
    { include: ["audit", "only_moves", "gaps", "congruence"], depth: 20 },
    {
      properties: {
        repertoire_id: string(),
        include: array({ type: "string", enum: ["audit", "only_moves", "gaps", "congruence"] }),
        depth: integer(1, 30),
        max_positions: integer(1, 300),
        min_cp_loss: integer(0),
        min_margin: integer(0),
  ...
  define(
    "prep_vs_opponent",
    "Compare a repertoire with an opponent's games and summarize preparation targets.",
    ["repertoire", "game", "network"],
    BOTH,
    { platform: "lichess", max_games: 30 },
    {
  ...
  define(
    "load_repertoire",
    "Parse a repertoire PGN and return a Node-host handle.",
    ["repertoire"],
    MCP,
    {},
    {
      properties: { pgn: string(), color: { type: "string", enum: ["white", "black"] } },
      mcpRequired: ["pgn", "color"],
    },
  ),
  define(
    "load_repertoire_from_file",
    "Load a repertoire PGN from the confined Node repertoire directory.",
    ["repertoire"],
    MCP,
    {},
    {
      properties: { path: string(), color: { type: "string", enum: ["white", "black"] } },
      mcpRequired: ["path", "color"],
    },
  ),
  define(
    "export_repertoire",
    "Serialize a Node repertoire handle to PGN.",
    ["repertoire", "artifact"],
    MCP,
    {},
    { properties: { repertoire_id: string() }, mcpRequired: ["repertoire_id"] },
  ),
  define(
    "export_repertoire_to_file",
    "Write repertoire PGN under the confined Node repertoire directory.",
    ["repertoire", "artifact"],
    MCP,
    {},
    {
      properties: { repertoire_id: string(), path: string() },
      mcpRequired: ["repertoire_id", "path"],
    },
  ),
  define(
    "propose_line",
    "Stage a validated SAN line for explicit user acceptance without mutating the repertoire.",
    ["repertoire", "action"],
    BROWSER,
    {},
    { properties: { moves: array(), comment: string() }, required: ["moves"] },
  ),
  define(
    "get_selected_subtree",
    "Retrieve bounded SAN lines for the currently selected repertoire subtree.",
    ["repertoire"],
    BROWSER,
    { max_plies: 80 },
    { properties: { max_plies: integer(1, 200) } },
  ),
  define(
    "get_document_pgn",
    "Retrieve the full current PGN only when an operation genuinely needs the artifact.",
    ["game", "repertoire", "artifact"],
    BROWSER,
    {},
    { properties: {} },
  ),
  define(
    ...
    {},
    {
      properties: {
        mode: {
          type: "string",
          enum: ["familiar-plans", "balanced", "versatile", "custom"],
          description:
            "Named preset. Selecting a preset alone restores that preset's preference defaults; combining it with preferences produces a custom profile.",
        },
        preferences: strategicFitIntentPreferences,
        rationale: string(
          "One or two sentences grounded in what the user actually said, shown next to the diff.",
          400,
        ),
      },
    },
  ...
  define(
    ...
    {},
    {
      properties: {
        ...
        rationale: string(
          "One or two sentences grounded in what the user actually said, shown next to the bounds.",
          400,
        ),
...
export const contractsForHost = (host: ToolHost) =>
  TOOL_CONTRACTS.filter((tool) => (tool.hosts as readonly ToolHost[]).includes(host));
...
export function jsonSchemaForTool(name: string, host: ToolHost): Record<string, unknown> | null {
  ...
  const hostProperties =
    host === "mcp" ? contract.input.mcpProperties : contract.input.browserProperties;
  const properties = Object.fromEntries(
    Object.entries({ ...contract.input.properties, ...hostProperties }).filter(
      ([key]) => !omitted.has(key),
    ),
  );
...
export type ArgumentsResult =
  ...
  | { ok: false; error: "invalid_arguments"; reason: string };
function fieldError(field: InputField, candidate: unknown, path: string): string | null {
  ...
  if (
    ...
    (candidate < (field.minimum ?? -Infinity) || candidate > (field.maximum ?? Infinity))
  )
    return `${path} is outside the allowed range`;
  if (typeof candidate === "string" && candidate.length > (field.maxLength ?? Infinity))
    return `${path} is outside the allowed length`;
  if (typeof candidate === "string" && field.pattern && !new RegExp(field.pattern).test(candidate))
    return `${path} has an invalid format`;
  if (field.enum && !field.enum.includes(candidate as never))
    return `${path} must be one of: ${field.enum.join(", ")}`;
  if (field.type === "array" && field.items) {
    if (
      (candidate as unknown[]).length < (field.minItems ?? 0) ||
      (candidate as unknown[]).length > (field.maxItems ?? Infinity)
    ) {
  ...
  if (field.type === "object") {
    ...
    for (const [key, nestedCandidate] of Object.entries(value)) {
      ...
      if (!nestedField && field.additionalProperties === false)
        return `unknown argument: ${path}.${key}`;
...
function strategicFitArgumentsError(value: Record<string, unknown>): string | null {
  ...
  for (const [list, identity] of [
    ["route_weights", "route_id"],
    ["decision_weights", "decision_id"],
  ] as const) {
  ...
  if (duplicateOverride)
    return `cohort_overrides contains duplicate override_id: ${duplicateOverride}`;
  ...
  if (duplicateAssessment)
    return `route_assessments contains duplicate route_id: ${duplicateAssessment}`;
...
function strategicFitRetrievalArgumentsError(value: Record<string, unknown>): string | null {
  ...
  if (typeof reportId !== "string" || reportId.trim().length === 0)
    return "report_id must not be blank";
...
function strategicFitPlanArgumentsError(value: Record<string, unknown>): string | null {
  ...
  for (const [index, section] of sections.entries()) {
    ...
    if (
      section.kind === "model-position" &&
      !(section.drill_ids as readonly unknown[] | undefined)?.length
    ) {
...
function replacementArgumentsError(value: Record<string, unknown>): string | null {
  ...
  if (value.contract !== "strategic-fit-replacement-v2")
    return "contract must be strategic-fit-replacement-v2";
  for (const key of [
    "replacement_request",
    "finding",
    "pivot",
  ...
  if (!id(record("replacement_request").request_id))
    return "replacement_request.request_id is required";
  ...
  if (
    ...
    return "replacement_request is incomplete";
  if (!id(record("finding").finding_id)) return "finding.finding_id is required";
  if (!["automatic", "user-selected"].includes(String(record("pivot").kind)))
    return "pivot.kind is invalid";
  if (
    !["familiar-plans", "balanced", "versatile", "custom"].includes(String(record("profile").mode))
  )
    return "profile.mode is invalid";
  const budget = record("budget");
  if (
    !Number.isInteger(budget.engine_depth) ||
    (budget.engine_depth as number) < 1 ||
    (budget.engine_depth as number) > 30 ||
    !Number.isInteger(budget.engine_multipv) ||
    (budget.engine_multipv as number) < 1 ||
    (budget.engine_multipv as number) > 10
  ) {
  ...
  if (
    ...
    (engine.depth as number) < 1 ||
    (engine.depth as number) > 30 ||
    !Number.isInteger(engine.multipv) ||
    (engine.multipv as number) < 1 ||
    (engine.multipv as number) > 10 ||
    typeof engine.allow_unavailable_evidence !== "boolean"
  ) {
    return "engine depth, multipv, and allow_unavailable_evidence are required";
  }
  if (typeof record("coverage").require_all_forcing_replies !== "boolean")
    return "coverage.require_all_forcing_replies is required";
  ...
  for (const [index, entry] of (value.retention as readonly Record<string, unknown>[]).entries()) {
    if (
      ...
      (entry.promote_candidate_to_mainline !== undefined &&
        typeof entry.promote_candidate_to_mainline !== "boolean")
    ) {
      return `retention[${index}] is invalid`;
    }
  }
  if ((value.candidate_ids as readonly unknown[]).some((candidate) => !id(candidate)))
    return "candidate_ids contains an invalid identity";
  ...
  if (
    ...
    !["white", "black"].includes(String(safety.repertoire_color))
  ) {
...
export function validateToolArguments(name: string, raw: unknown, host: ToolHost): ArgumentsResult {
  ...
  if (!(contract.hosts as readonly ToolHost[]).includes(host))
    return {
      ...
      reason: `${name} is not available on the ${host} host`,
    };
  ...
  for (const key of (schema.required as string[] | undefined) ?? [])
    if (!(key in value))
      return { ok: false, error: "invalid_arguments", reason: `missing required argument: ${key}` };
  for (const [key, candidate] of Object.entries(value)) {
    const field = properties[key];
    if (!field)
      return { ok: false, error: "invalid_arguments", reason: `unknown argument: ${key}` };
