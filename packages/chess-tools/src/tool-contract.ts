...
export type ToolInput = { properties: Readonly<Record<string, InputField>>; mcpProperties?: Readonly<Record<string, InputField>>; required?: readonly string[]; mcpRequired?: readonly string[] };
...
export const TOOL_CONTRACTS = [
  ...
  define("audit_repertoire_moves", "Engine-check prescribed moves tree-wide and rank findings by centipawn loss.", ["repertoire", "engine"], MCP, { depth: 14, min_cp_loss: 50 }, { properties: { repertoire_id: string(), depth: integer(1, 30), min_cp_loss: integer(0), max_positions: integer(1, 60), limit: integer(1, 50) }, mcpRequired: ["repertoire_id"] }),
  define("find_only_moves", "Find positions where the best move clearly exceeds the second choice.", ["repertoire", "engine", "artifact"], MCP, { depth: 14, min_margin: 100, limit: 25 }, { properties: { repertoire_id: string(), depth: integer(1, 30), min_margin: integer(0), max_positions: integer(1, 300), limit: integer(1, 100), lines_limit: integer(1, 50), export_path: string() }, mcpRequired: ["repertoire_id"] }),
  define("find_structures", "Search repertoire leaves by structure, center, themes, or color complex.", ["repertoire"], MCP, { limit: 30 }, { properties: { repertoire_id: string(), structure: string(), min_confidence: { type: "number", minimum: 0, maximum: 1 }, center: { type: "string", enum: ["tense", "locked", "open", "semi-open"] }, themes: array({ type: "string", enum: ["fianchetto_white", "fianchetto_black", "minority_attack_white", "minority_attack_black", "flank_vs_center"] }), color_complex: { type: "string", enum: ["light", "dark"] }, limit: integer(1, 100) }, mcpRequired: ["repertoire_id"] }),
  ...
  define("export_annotated_repertoire", "Run analyses and produce a cloned, annotated repertoire PGN.", ["repertoire", "engine", "artifact"], MCP, { include: ["audit", "only_moves", "gaps", "congruence"] }, { properties: { repertoire_id: string(), include: array({ type: "string", enum: ["audit", "only_moves", "gaps", "congruence"] }), depth: integer(1, 30), max_positions: integer(1, 300), min_cp_loss: integer(0), min_margin: integer(0), min_severity: { type: "string", enum: ["low", "medium", "high"] }, export_path: string() }, mcpRequired: ["repertoire_id"] }),
  define("prep_vs_opponent", "Compare a repertoire with an opponent's games and summarize preparation targets.", ["repertoire", "game", "network"], MCP, { platform: "lichess", max_games: 30 }, { properties: { repertoire_id: string(), username: string(), platform: { type: "string", enum: ["lichess", "chesscom"] }, max_games: integer(1, 100), year: integer(), month: integer(1, 12) }, mcpRequired: ["repertoire_id", "username"] }),
...
export function jsonSchemaForTool(name: string, host: ToolHost): Record<string, unknown> | null {
  ...
  const properties = Object.fromEntries(Object.entries({ ...contract.input.properties, ...(host === "mcp" ? contract.input.mcpProperties : {}) }).filter(([key]) => !omitted.has(key)));
