...
function subtreePaths(tree: GameTree, rootPath: readonly string[]): string[][] {
  ...
  while (pending.length > 0) {
    ...
    node.children.forEach((child, index) => pending.push({
...
test("safe replacement archives before pruning and returns exact archive evidence", () => {
  ...
  const pruneDiff = result.output.result.preview.operation_diffs.find((diff) => diff.operation_id === prune.operation_id)!;
  assert.deepEqual(pruneDiff.removed_paths, subtreePaths(values.tree, prune.target.source_san_path));
});
...
test("all six Task 8.8 operation kinds execute, including annotation, link, and reorder", () => {
  ...
  const safety = simulateReplacementSafety({ source_tree: tree, request: base.fixture.request, scoring });
  ...
  const replacementKinds = new Set(constructed(replacement).operations.map((operation) => operation.kind));
  assert.ok(replacementKinds.has("archive-subtree"));
  assert.ok(replacementKinds.has("prune-subtree"));
  assert.deepEqual(new Set([...kinds, ...replacementKinds]), new Set(TASK_8_8_CHANGE_OPERATION_KINDS));
});
...
test("middle-operation failure rolls back and exposes no result tree or partial operation result", () => {
  ...
  const changed = { ...valid, operations: operations.map((operation, sequence) => ({ ...operation, sequence })) };
  const sourcePgn = values.tree.toPgn();
...
test("affected paths equal exact union of operation diffs", () => {
  ...
  const paths = preview.operation_diffs.flatMap((diff) => [
    ...diff.added_paths,
    ...diff.removed_paths,
  ...
  assert.deepEqual(preview.affected_paths.map((path) => path.join("\u001f")), [...new Set(paths)].sort());
});
...
test("compatible comments move only across semantic equivalence; incompatible comments remain archived", () => {
  ...
  const simulated = values.safety.candidates.find((candidate) => candidate.candidate_id === values.candidate.candidate_id)!;
  ...
  const compatible = expansion.subtree.edges.map((edge) => {
  ...
  (values.tree.nodeAt(sourceIndex!) as unknown as { data: { comments?: string[] } }).data.comments = ["Compatible note"];
  const base = constructed(values);
  const annotation = base.operations.find((operation) =>
  ...
  assert.ok((result.tree.nodeAt(targetIndex!) as unknown as { data: { comments?: string[] } }).data.comments?.includes("Compatible note"));

  const injected = clone(base);
  const injectedAnnotation = injected.operations.find((operation) => operation.operation_id === annotation.operation_id)!;
  if (injectedAnnotation.kind === "preserve-annotation") injectedAnnotation.comments = ["Injected note"] as never;
  assert.equal(apply(values, injected).output.failure?.code, "annotation-not-equivalent");

  const malformed = clone(base);
  const malformedAnnotation = malformed.operations.find((operation) => operation.operation_id === annotation.operation_id)!;
  if (malformedAnnotation.kind === "preserve-annotation") malformedAnnotation.comments = [null] as never;
  ...
  assert.match(replaced.output.result.preview.archive_payloads[0]!.pgn, /Do not copy across structure/);
...
test("semantic duplicates merge and canonical transposition links do not multiply editorial routes", () => {
  ...
  assert.equal(result.output.result.preview.operation_diffs
    .filter((diff) => diff.kind === "add-subtree").flatMap((diff) => diff.added_paths).length, 0);
});
...
test("stale revision, semantic path, Task 8.7 result, identity, version, and provenance are rejected", () => {
  ...
  staleSafety.candidates[0]!.provenance = [...staleSafety.candidates[0]!.provenance, {
    source_id: "forged",
  ...
  assert.equal(apply(values, { ...valid, request_id: "request:wrong" }).output.failure?.code,
    "change-set-identity-mismatch");
  assert.equal(apply(values, { ...valid, replacement_schema_version: "0.0.0" } as never).output.failure?.code,
...
test("illegal operation, invalid ordering, and pruning without confirmation are rejected", () => {
  ...
  wrongOrder.operations = [...wrongOrder.operations].reverse().map((operation, sequence) => ({ ...operation, sequence })) as never;
  ...
  const wrongArchivePrune = wrongArchiveTarget.operations.find((operation) => operation.kind === "prune-subtree")!;
  const wrongArchiveAdd = wrongArchiveTarget.operations.find((operation) => operation.kind === "add-subtree")!;
...
test("Black ownership and White-POV transport survive complete change-set serialization", () => {
  ...
  const tree = GameTree.fromPgn(PGN);
  const safety = simulateReplacementSafety({ source_tree: tree, request: fixture.request, scoring });
  const result = constructAndApplyReplacementChangeSet({
    source_tree: tree,
  ...
  assert.equal(result.output.result.preview.objective_quality_after.white_pov_evaluation_cp !== null, true);
  assert.equal(result.output.result.preview.objective_quality_after.repertoire_pov_evaluation_cp !== null, true);
  assert.equal(result.output.result.preview.objective_quality_after.repertoire_pov_loss_from_best_cp, 20);
...
test("full inputs and Task 8.3-8.7 evidence remain immutable; outputs are deterministic", () => {
  ...
  const shuffled = { ...secondConstructed.change_set, operations: [...secondConstructed.change_set.operations].reverse() };
  ...
  assert.equal(isDeepStrictEqual(first.output.result.preview, second.output.result.preview), true,
  ...
  assert.equal(isDeepStrictEqual(JSON.parse(JSON.stringify(first.output)), first.output), true,
...
test("operation, result, and error enums are exhaustive and duplicate-free", () => {
  assert.deepEqual(TASK_8_8_CHANGE_OPERATION_KINDS, [
    "add-subtree", "link-transposition", "preserve-annotation", "archive-subtree",
    "prune-subtree", "reorder-variations",
  ]);
  ...
  assert.deepEqual(REPLACEMENT_CHANGE_SET_RESULT_STATUSES,
    ["previewed", "applied", "rejected", "failed", "stale"]);
  assert.deepEqual(REPLACEMENT_OPERATION_RESULT_STATUSES, ["applied", "skipped", "failed"]);
  for (const values of [TASK_8_8_CHANGE_OPERATION_KINDS, REPLACEMENT_CHANGE_SET_ERROR_CODES,
    REPLACEMENT_CHANGE_SET_RESULT_STATUSES, REPLACEMENT_OPERATION_RESULT_STATUSES]) {
