...
> **Note:** The MCP server is a Node.js/TypeScript implementation (`apps/mcp-server`, all 34 tools). `.mcp.json` launches it directly — no container, no port, no host Stockfish install (the engine ships as a bundled wasm).
...
A **workflow mode** selector in the chat panel (General / Repertoire / Game review / Position / Annotate) injects the matching playbook into the system prompt — the PWA counterpart of the Claude Code skills (`apps/ui/src/llm/workflows.ts`), telling the assistant which tools to call in what order plus the grounding rules. The skills under `.claude/skills/` remain the Claude Code version (handle-based); the PWA prompts are adapted to its handle-free, current-tree tools. The mode also filters which tool schemas are sent (they are re-sent on every round of the tool loop): each specific mode ships only the tools its playbook uses; General keeps the full set.

The **Lichess opening-explorer tools** (`position_popularity`, `find_theory_depth`, `find_repertoire_gaps popularity=true`) work in the chat too — `explorer.lichess.org` allows CORS, so the browser talks to it directly. The explorer requires a Lichess login: paste a [personal API token](https://lichess.org/account/oauth/token) (no scopes needed) into Settings; without one those tools return `explorer_auth_required`.
...
| `position_popularity` | FEN, db?, top_moves? | Lichess opening-explorer stats — what humans actually play here: per-move frequency + win rates (white-POV), total games, opening name. `db` = `lichess` (online games, 1800+ blitz/rapid/classical, default) or `masters` (OTB). Requires `LICHESS_TOKEN` |
...
| `find_structures` | repertoire_id, structure?, center?, themes?, color_complex?, min_confidence?, limit | Structural position **search** (engine-free): every line whose final position matches the query — "show every line reaching an IQP / fianchetto / locked-center position". Criteria are AND-ed (at least one required); `structure` matches the 19-class classifier, `themes` the boolean theme tags, `center` the pawn-center state. The query complement of `get_structural_profile` |
...
| `find_repertoire_gaps` | repertoire_id, depth, min_severity, limit, max_positions, popularity?, popularity_db? | Engine scan for completeness: at every opponent-to-move node you already answer, flags strong opponent replies the tree doesn't cover, each with drill-down path + severity. `popularity=true` (needs `LICHESS_TOKEN`) annotates each gap with how often the move is actually played (`played_pct`/`played_games` from the opening explorer) and re-ranks by frequency within each severity tier — triage hits the holes you'll actually face |
| `audit_repertoire_moves` | repertoire_id, depth, min_cp_loss, limit, max_positions | Engine check of **your own** prescribed moves, tree-wide: every your-turn position is searched (multipv-2) and each repertoire move scored vs the engine's best; findings ranked worst-first by `cp_loss` with SAN drill-down path, `classification` (good/inaccuracy/mistake/blunder) and `best_margin` (best − second line, the only-move signal). Complement of `find_repertoire_gaps` |
| `find_only_moves` | repertoire_id, depth, min_margin, limit, max_positions, lines_limit, export_path? | Criticality tagging: your-turn positions where the engine best stands alone (best − second ≥ `min_margin`, default 100cp) — the "only move" spots where misremembering is punished. Findings ranked by margin; `lines[]` ranks leaf lines by only-move density (sharpest lines to drill). `prescribed_is_best=false` flags a sharp position whose repertoire move isn't the engine best (fix via `audit_repertoire_moves` first). `export_path` writes the full tagged set as a flashcard CSV (`front,back,fen,margin`, Anki-importable), confined to `REPERTOIRE_DIR` |
...
| `export_annotated_repertoire` | repertoire_id, include?, depth, max_positions, min_cp_loss, min_margin, min_severity, export_path? | Embed analysis findings as PGN comments/NAGs at the flagged nodes — portable to any board GUI. `include` picks the sources: `audit` (NAG + cp-loss comment on your bad prescribed moves), `only_moves` (drill notes), `gaps` (uncovered replies), `congruence` (thematic outliers; engine-free) — default all four; engine sources share searches via the eval cache. `export_path` writes the PGN to a file (confined to `REPERTOIRE_DIR`), else it returns inline |
...
| `repertoire_vs_history` | repertoire_id, username, platform, max_games, year?, month? | Compare prep against real games: coverage %, player deviations (your off-book moves), uncovered opponent moves. The walk reports **every** departure per game, not just the first — it keeps matching positions by transposition key past a departure, so a game that leaves book early still surfaces a later novelty |
| `prep_vs_opponent` | repertoire_id, username, platform, max_games, year?, month? | Match prep against a named opponent: fetches their games on the color they'd face this repertoire from, reports how often your lines will actually come up (coverage + per-opening `lines[]` with hit/win/draw/loss rates from their own results) and their habitual moves your tree doesn't cover |
| `find_theory_depth` | repertoire_id, db?, min_games?, max_positions? | Where each line leaves known theory: walks the tree querying the opening explorer and reports, per line, the ply at which game counts collapse below `min_games` (default 100 lichess / 5 masters) — the point where memorization stops paying and prep becomes original. Transposition-deduped and pruned below each exit, so queries ≈ in-theory positions only; capped by `max_positions` (network-bound, ~1 query/s). Requires `LICHESS_TOKEN` |

Structural analysis recognizes **19 canonical pawn structures** — IQP, Carlsbad, Maroczy, French, Stonewall, King's Indian, Benoni, Closed Sicilian, Hanging pawns, Caro-Kann, Slav, Grünfeld Centre, Nimzo-Grünfeld, Hedgehog, Najdorf, Scheveningen, Symmetric Benoni, Lopez, and Benko — each gated on a core skeleton with graduated confidence (a position missing a peripheral pawn still classifies), the open-Sicilian family scored bidirectionally (reversed-English positions included), else `unknown`. The canon is traced to Flores Rios *Chess Structures* and Soltis *Pawn Structure Chess*; every scorer is validated against an engine-verified canonical FEN (`scripts/structure-accuracy.mjs`). Beyond the named class, every position also carries always-on **theme tags** (fianchetto, space, wing-majority, minority-attack, flank-vs-centre, colour-complex) — these stay informative even when the class is `unknown` (e.g. fianchetto systems), and the aggregate profile rolls them up across all leaves. Opening names come from the [lichess-org/chess-openings](https://github.com/lichess-org/chess-openings) dataset (CC0).

Engine-backed tools accept `depth` (integer, clamped 1–30; per-tool default 12–16). `find_pruning_transpositions` additionally accepts `movetime_ms` (ms per position) as a time-based alternative — a tighter knob for sharp positions or slower hardware — and `confirm_depth` to deep-confirm each line's best-eval re-route. Its candidate-node pre-filter (only positions with an actual cross-branch transposer reach the engine) plus a transposition-keyed scan memo keep a full-tree scan cheap — and the engine-free pre-pass itself is cached per repertoire handle, so cursor chunks and repeat scans don't re-pay it. Chunking (`leaf_start`/`leaf_count`) is for progress display, not coverage.
...
| `ENGINE_POOL_SIZE` | `min(cores, 4)` | Stockfish child processes for parallel searches (clamped 1–8); `0` forces the single in-process engine |
| `LICHESS_TOKEN` | unset | [Personal API token](https://lichess.org/account/oauth/token) (no scopes needed) for the opening-explorer tools (`position_popularity`, `find_theory_depth`, gap `popularity`) — Lichess requires a login for the explorer since March 2026; without it those tools return `explorer_auth_required` |
...
├── apps/
│   ├── mcp-server/          # Node MCP server (@modelcontextprotocol/sdk, stdio) — 34 tools + stockfish wasm
...
├── docs/design/             # design specs: MCP principles, repertoire architecture, UI
...
- [x] **`classify_structure` expansion** — now **19 source-traced structures** (added Hanging pawns, Caro-Kann, Slav, Grünfeld Centre, Nimzo-Grünfeld, Hedgehog, Najdorf, Scheveningen, Symmetric Benoni, Lopez, Benko) with graduated core+bonus confidence and bidirectional open-Sicilian scoring, plus always-on **theme tags** (A) rolled up in the aggregate profile. Each scorer is validated against an engine-verified canonical FEN (`scripts/structure-accuracy.mjs`).
