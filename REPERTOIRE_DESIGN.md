...
- `classify_structure(board) -> dict` → `{structure_class, confidence}` where
  `structure_class ∈ {IQP, Carlsbad, Maroczy, unknown}` (D2). Rule-based pattern match on
  the primitives; never forces a label — weak match → `unknown` with low confidence.
