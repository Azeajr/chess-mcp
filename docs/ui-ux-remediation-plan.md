...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | **Linting and formatting are now available.** Root scripts provide `pnpm lint` (ESLint 9 flat config with type-aware checks for package source) and `pnpm format:check` (Oxfmt). The repository was brought into compliance after this plan was drafted. CI does not yet run either command.                                                                                                                     | Every package's Definition of Done includes `pnpm lint` and `pnpm format:check` alongside `pnpm -r typecheck`, `pnpm docs:check`, and `pnpm check:skills`. `WP-000` wires the static checks into CI; it does not redesign their rules or perform another repository-wide formatting pass.                     |
...
- ESLint and Oxfmt are available locally; neither is yet enforced in CI (D1).
...
Gates run **before** their package's implementation begins, not before its merge. A gate that returns the default costs one session, not a phase. **Exception:** AG-3 is the validation gate for WP-011 itself because its required screen-reader evidence can only be collected against the implemented move tree; WP-011 may start after DV-2 and must not be marked complete until AG-3 is resolved.
