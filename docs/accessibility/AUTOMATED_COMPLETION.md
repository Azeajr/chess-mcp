...
| Real screen readers convey tree context, item level, and expanded/collapsed state.   | Guidepup drives real NVDA on Windows and VoiceOver on macOS. The collector slices the utterance log per command; a deterministic matcher requires the scenario's move name plus tree-context, level, and state vocabulary. Native semantics remain independently required in all three browser accessibility trees. VoiceOver's web mapping omits native treeitem level/state from both keyboard-focus and cursor descriptions, so the item's accessible name supplies those same facts as the objective spoken proxy. |
...
VoiceOver observations. The Markdown report is diagnostic only. Native role vocabulary is not a
VoiceOver completion claim: real runs showed that VoiceOver reports the focused web treeitem as a
"group" even when browser AX and NVDA expose the correct role. The retained user-facing claim is
that VoiceOver audibly conveys repertoire-tree context, variation level, and branch state; exact
accessible-name and utterance assertions prove that proxy without pretending the platform spoke a
native role it did not.
