...
Status: **shipped**. This doc is the design rationale of record; current state lives in `README.md`
and `AGENTS.md`. Companion: `MCP_DESIGN.md` (server-wide design principles — every rule below traces
back to it).

> Written against the original Python server; the shipped code is **TypeScript** (`packages/chess-tools`
> + `apps/mcp-server`). The decisions below are language-agnostic — file/line references such as
> `repertoire.py:NN` are historical, superseded by the TS port.
...
Closed error-code set additions (the full current set is enumerated in `MCP_DESIGN.md`):
...
Two linked goals layered on the shipped base (Part I above):
...
- It preserves the shipped contract verbatim — *each handle = one immutable tree*, every
  tool a pure function of `(repertoire_id, args)` (`MCP_DESIGN.md` Stateless). A
