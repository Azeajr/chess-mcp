...
**Blocking gates.** None. DV-2 is the fixed ARIA tree-navigation decision. **Completion gate.** AG-3, resolved only by its deterministic browser and real-AT command.
...
- DV-2 is fixed to the ARIA tree pattern above. Automation asserts the exact key/state transition table; preference between navigation models is not a completion claim.
